...
function tbAct:OnGetInvite(nInviteId, szName)
    ...
    local szMsg = string.format("%s邀請你一同前往溫馨小屋, 是否願意？\n(%%d秒後自動關閉)", szName)
    me.MsgBox(szMsg, {{"同意", fnAgree}, {"拒絕", fnDisagree}}, nil, 20, fnClose)
end
...
function tbAct:OnGetDazuoInvite(nInviteId, szName)
    ...
    local szMsg = string.format("%s邀請你共賞玫瑰煙火, 是否願意與他並肩席坐，同賞美景？\n(%%d秒後自動關閉)", szName)
    me.MsgBox(szMsg, {{"同意", fnAgree}, {"拒絕", fnDisagree}}, nil, 20, fnClose)
end
