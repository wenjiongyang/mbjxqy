...
function Npc:OnDialog(szClassName, szParam)
	...
	local szDefaultText = Lib:IsEmptyStr(him.szDefaultDialogInfo) and "你找我什麼事？" or him.szDefaultDialogInfo
	...
	if ActivityQuestion:IsCurQuestionNpc(me.nMapTemplateId, him.nTemplateId) then
		if bTaskFlag then
			table.insert(tbDialogInfo.OptList, {Text = "答題", Type = "Script", Callback = ActivityQuestion.BeginAnswer, Param = {ActivityQuestion, me.nMapTemplateId, him.nTemplateId}})
		else
...
function Npc:ShowDefaultDialog(szClassName, szParam)
	if  Lib:IsEmptyStr(szClassName) or not self.tbClass[szClassName] then
		...
		if not szText then
			szText = Lib:IsEmptyStr(him.szDefaultDialogInfo) and "你找我什麼事？" or him.szDefaultDialogInfo;
		end
