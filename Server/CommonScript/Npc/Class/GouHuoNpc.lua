	...
	if tbTmp.nCanUseJIu ~= 0 then
		...
		if tbTmp.nQuotiety ~= nQuotiety or tbTmp.szJiuName ~= szJiuName then
			...
			if szJiuName and nQuotiety and nJiuMax then
				if nTeamId ~= 0 then
					szLastGuohuoTipContent = string.format("現在有%d名隊伍成員分享了%s，獲得%d%%的篝火經驗", nJiuMax, szJiuName, nQuotiety)
					...
					szLastGuohuoTipContent = string.format("目前處於飲酒狀態，獲得%d%%的篝火經驗", nQuotiety)
					if pOwnner.szLastGuohuoTipContent ~= szLastGuohuoTipContent then
						pOwnner.Msg(szLastGuohuoTipContent)
					end
				end
			else
				szLastGuohuoTipContent = "酒勁已過，獲得100%的篝火經驗"
				if pOwnner.szLastGuohuoTipContent ~= szLastGuohuoTipContent then
	...
	if nAnnouce == 1 then
		if nJiuMax == 0 then
			if szEventJiuName then
				...
				szMsg = string.format(XT("沒有使用酒，"));
			end
			szMsg = string.format(XT("%s篝火現在獲得經驗的倍數為[1AB8E1]%s[-]。"), szMsg, nQuotiety)
		else
			if szEventJiuName then
				szMsg = string.format(XT("隊友正在分享%s和%s，經驗加成為%s％"), szJiuName, szEventJiuName, nQuotiety)
			else
				szMsg = string.format(XT("隊友正在分享%s，經驗加成為%s％"), szJiuName, nQuotiety)
			end
