local tbNpc = Npc:GetClass("CommerceGather")

function tbNpc:OnCreate(szParam)
	local bMature, nMatureId, nUnMatureId = CommerceTask:ResolveGatherParam(szParam);
	self.tbMatureTime = self.tbMatureTime or {};
	if not bMature then
		local nMatureTime = CommerceTask:GetRefreshInterval(nMatureId);
...
function tbNpc:EnterMature(nNpcId, nMatureId)
	...
	if not pNpc or pNpc.IsDelayDelete() then
		Log("[CommerceGather EnterMature Fail], Npc Is Nil", nNpcId, nMatureId)
		return
	end
...
function tbNpc:OnDialog(szParam)
	...
	if not CommerceTask:GatherThingInTask(me, nMatureId) then
		me.CenterMsg("沒有該採集物的任務")
		return;
	end
	
	if bMature then
		GeneralProcess:StartProcess(me, 6 * Env.GAME_FPS, "採集中...", self.EndProcess, self, me.dwID, him.nId, nMatureId, nUnMatureId);
	else
		local nMatureTime = self.tbMatureTime[him.nId] or (CommerceTask:GetRefreshInterval(nMatureId) + GetTime())
		local nSurviveSec = nMatureTime - GetTime();
		nSurviveSec = math.max(nSurviveSec, 1)
		local szTip = string.format("%d分鐘後成熟",  math.ceil(nSurviveSec / 60));
		me.CenterMsg(szTip);
	end
end
...
function tbNpc:EndProcess(nPlayerId, nNpcId, nMatureId, nUnMatureId)
	...
	local pNpc = KNpc.GetById(nNpcId);
	if not pNpc or pNpc.IsDelayDelete() then
		Dialog:CenterMsg(pPlayer, "已被其他人搶先採走啦");
		return;
 	end
	...
	self.tbMatureTime[pNpc.nId] = nil;
 	...
 	local bFinish = CommerceTask:AddGatherTing(pPlayer, nMatureId);
 	CommerceTask:SyncCommerceData(pPlayer);
 	if bFinish then
 		pPlayer.CallClientScript("Ui:OpenWindow", "CommerceTaskPanel");
 	end
end
