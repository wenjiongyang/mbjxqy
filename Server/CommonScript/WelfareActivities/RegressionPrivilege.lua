...
RegressionPrivilege.DOUBLE_ACT            = {
    Boss                                  = {nSaveKey = 24, nMaxSaveKey = 37, nDayPer = 2.5, szMsg = "(回歸特權額外獲得[FFFE0D]%d貢獻[-])",
                                             tbUiInfo = {szTitle = "武林盟主獎勵翻倍", szContent = "武林盟主單人獎勵貢獻翻倍", szBtnContent = "查看活動", bCloseMyself = true, szBtnUrl = "[url=openwnd:test, CalendarPanel, 1, 1]"}},
    CommerceTask                          = {nSaveKey = 25, nMaxSaveKey = 38, nDayPer = 5, szMsg = "商會任務回歸特權額外獎勵",
                                             tbUiInfo = {szTitle = "商會任務獎勵翻倍", szContent = "商會任務獎勵的銀兩及藏寶圖或卷軸獎勵加倍", szBtnContent = "查看活動", bCloseMyself = true, szBtnUrl = "[url=openwnd:test, CalendarPanel, 1, 2]"}},
    HeroChallenge                         = {nSaveKey = 26, nMaxSaveKey = 39, nDayPer = 0.5, szMsg = "英雄挑戰回歸特權額外獎勵",
                                             tbUiInfo = {szTitle = "英雄挑戰獎勵翻倍", szContent = "英雄挑戰每次挑戰獲得兩次隨機獎勵", szBtnContent = "查看活動", bCloseMyself = true, szBtnUrl = "[url=openwnd:test, CalendarPanel, 1, 23]"}},
    KinGather                             = {nSaveKey = 27, nMaxSaveKey = 40, nDayPer = 1.25, szMsg = "幫派答題回歸特權額外獎勵",
                                             tbUiInfo = {szTitle = "烤火答題獎勵翻倍", szContent = "幫派烤火答題時獲得雙倍貢獻", szBtnContent = "查看活動", bCloseMyself = true, szBtnUrl = "[url=openwnd:test, CalendarPanel, 1, 18]"}},
    RankBattle                            = {nSaveKey = 28, nMaxSaveKey = 41, nDayPer = 5, szMsg = "武神殿回歸特權額外獎勵",
                                             tbUiInfo = {szTitle = "武神殿獎勵翻倍", szContent = "武神殿領取獎勵時額外獲得一個同樣的武神寶箱", szBtnContent = "查看活動", bCloseMyself = true, szBtnUrl = "[url=openwnd:test, CalendarPanel, 1, 9]"}},
}
RegressionPrivilege.DayTargetEXT          = {nSaveKey = 29, nMaxSaveKey = 42, nDayPer = 1,
                                             tbUiInfo = {szTitle = "每日目標額外獎勵", szContent = "完成每日目標可額外獲得大量經驗、銀兩、貢獻獎勵", szBtnContent = "前去查看", bCloseMyself = true, szBtnUrl = "[url=openwnd:test, CalendarPanel, 3]"}}

--充值特权
RegressionPrivilege.RECHARGE_AWARD        = { --充值额外奖励（nSaveKey不能修改）
                               { nSaveKey = 30, nShowPro = 2, nDayPer = 3,  nRechargeIdx = 1, szContent = "0.99$回歸元寶禮包"},
                               { nSaveKey = 31, nShowPro = 1, nDayPer = 30, nRechargeIdx = 2, szContent = "3.99$回歸元寶禮包"},
                               { nSaveKey = 32, nShowPro = 5, nDayPer = 6,  nRechargeIdx = 3, szContent = "0.99$超值福利禮包"},
                               { nSaveKey = 33, nShowPro = 4, nDayPer = 2,  nRechargeIdx = 4, szContent = "0.99$超值回歸禮包"},
                               { nSaveKey = 34, nShowPro = 3, nDayPer = 4,  nRechargeIdx = 5, szContent = "3.99$超值豪華禮包"},
}
...
RegressionPrivilege.Outline_Days    = 15    --离线天数，必须大于等于该等级才算回归
...
RegressionPrivilege.tbTianJian = {nItemTID = 2682, nPrice = 1500, nOriginalPrice = 5000} --打折天剑令
...
function RegressionPrivilege:IsCloseMarketStall(pPlayer)
    ...
    return self:IsTriggerByAct(pPlayer), "擺攤暫時關閉，請領取老玩家回歸特權"
end
...
function RegressionPrivilege:CheckFreeGainExt(pPlayer, nId)
    if nId == self.WAIZHUANG then
        if pPlayer.GetUserValue(self.GROUP, self.OLD_VIPLEVEL) < self.nClearItemVipLv then
            return false, string.format("需達到VIP%d才可領取", self.nClearItemVipLv)
        end
    elseif nId == self.YUANQI then
        return pPlayer.GetUserValue(self.GROUP, self.YUANQI_AWARD) > 0, "尚未開放"
    end
    return true 
end
