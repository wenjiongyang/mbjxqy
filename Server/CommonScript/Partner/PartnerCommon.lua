...
function Partner:Init()
	...
	local tbFile = LoadTabFile("Setting/Partner/PartnerTemplate.tab", "dsdddsds", nil, {"Id", "szGrowthType", "StoneIcon", "StoneValue", "Value", "Info", "Hide", "ShowTimeFrame"});
...
function Partner:CheckCanUseSkillBook(pPlayer, nPartnerId, nItemId)
	...
	if tbSkillSetting.nSeries > 0 and nSeries ~= tbSkillSetting.nSeries then
		return false, string.format("%s與此書五行不符！", szName);
	end
		...
		if nCurSkillId > 0 then
			...
			if tbSSetting.nType == tbSkillSetting.nType then
				if tbSSetting.nLevel >= tbSkillSetting.nLevel then
					return false, tbSSetting.nLevel == tbSkillSetting.nLevel and "已經擁有相同的技能了" or "已經擁有同類高階的技能了";
				else
...
function Partner:CheckCanUseProtentialItem(pPlayer, nPartnerId, nProtentialType)
	...
	if not szProtentialType then
		return false, "請選擇要提升的資質";
	end

	local pPartner = pPlayer.GetPartnerObj(nPartnerId);
	if not pPartner then
		return false, "目標同伴不存在";
	end
	...
	if nPValue >= nLimitProtential then
		if bMax then
			return false, string.format("%s潛能已達上限", self.tbProtentialName[nProtentialType]);
		else
			return false, string.format("%s已達目前上限，突破後才可提升", self.tbProtentialName[nProtentialType]);
		end
	end
...
function Partner:CheckReinitPartner(pPlayer, nPartnerId)
	...
	if not nCount or nCount < nCost then
		return false, "洗髓丹不足";
	end
	...
	return true, "", pPartner, nCost, tbAward, nQualityLevel, nProtentialValue;
end
...
function Partner:CheckCanDecomposePartner(pPlayer, tbPartnerList)
	...
	return true, "", tbAward, szLogInfo, nUseItemProtentialValue;
end
...
function Partner:GetResis(tbAttribInfo, szType)
	...
	return math.floor(nSeries * (nSeriesP + 100) / 100 + (tbAttribInfo.nAllSeries or 0) * (100 + nAllSeriesP) / 100);
end
...
function Partner:GetIgnoreResis(tbAttribInfo, szType)
	local nSeries = (tbAttribInfo.tbBaseAttrib["ignore_" .. szType .. "_resist_v"] or {tbValue = {0, 0}}).tbValue[2];
	return nSeries;
end
...
Partner.tbAllAttribDef =
{
	...
	{"攻擊力",		Partner.GetBaseAttack},
	{"體力",		"nVitality", "vitality_v"},
	{"敏捷",		"nDexterity", "dexterity_v"},
	{"力量",		"nStrength", "strength_v"},
	{"靈巧",		"nEnergy", "energy_v"},
	{"體質成長",	"Protential", "Vitality"},
	{"敏捷成長",	"Protential", "Dexterity"},
	{"力量成長",	"Protential", "Strength"},
	{"靈巧成長",	"Protential", "Energy"},
	{"生命恢復",	"recover_life_v"},
	...
	{"閃避",		Partner.GetValueByBaseValue, "nDefense", "defense_v", "defense_p"},
	{"忽略閃避",	"ignore_defense_v"},
	{"會心機率",	Partner.GetDeadlyStrike},
	{"會心傷害",	Partner.GetDeadlyStrikeDamage},
	{"抗會心機率",	"weaken_deadlystrike_v"},
	{"會心免傷",	"weaken_deadlystrike_damage_p"},
	{"眩暈機率",	"state_stun_attackrate"},
	{"遲緩機率",	"state_slowall_attackrate"},
	{"致纏幾率",	"state_zhican_attackrate"},
	{"麻痹機率",	"state_palsy_attackrate"},
	{"抗受傷機率",	Partner.GetValueWithAllValue, "state_hurt_resistrate", "resist_allseriesstate_rate_v"},
	{"抗眩暈機率",	Partner.GetValueWithAllValue, "state_stun_resistrate", "resist_allseriesstate_rate_v"},
	{"抗遲緩機率",	Partner.GetValueWithAllValue, "state_slowall_resistrate", "resist_allseriesstate_rate_v"},
	{"抗致纏幾率",	Partner.GetValueWithAllValue, "state_zhican_resistrate", "resist_allseriesstate_rate_v"},
	{"抗麻痹機率",	Partner.GetValueWithAllValue, "state_palsy_resistrate", "resist_allseriesstate_rate_v"},
	{"抗受傷時間",	Partner.GetValueWithAllValue, "state_hurt_resisttime", "resist_allseriesstate_time_v"},
	{"抗眩暈時間",	Partner.GetValueWithAllValue, "state_stun_resisttime", "resist_allseriesstate_time_v"},
	{"抗遲緩時間",	Partner.GetValueWithAllValue, "state_slowall_resisttime", "resist_allseriesstate_time_v"},
	{"抗致纏時間",	Partner.GetValueWithAllValue, "state_zhican_resisttime", "resist_allseriesstate_time_v"},
	{"抗麻痹時間",	Partner.GetValueWithAllValue, "state_palsy_resisttime", "resist_allseriesstate_time_v"},
}

Partner.tbJingMaiExtAttribDef =
{
	{"忽略金系抗性", Partner.GetIgnoreResis, "metal"},
	{"忽略木系抗性", Partner.GetIgnoreResis, "wood"},
	{"忽略水系抗性", Partner.GetIgnoreResis, "water"},
	{"忽略火系抗性", Partner.GetIgnoreResis, "fire"},
	{"忽略土系抗性", Partner.GetIgnoreResis, "earth"},
	{"抵擋角色傷害", Partner.GetAbsBaseValueWhithP, "playerdmg_npc_p"},
	{"對怪物傷害", Partner.GetBaseValueWhithP, "damage4npc_p"},
};
...
function Partner:OnCreatePartnerNpc(pPartner, pNpc, bByScript)
	...
	if pPartner.GetAwareness() == 1 then
		...
		if tbAwareness and tbAwareness.nAwarenessSkillId and tbAwareness.nAwarenessSkillId > 0 then
			...
			if not string.find(pNpc.szName, "覺醒") then
				pNpc.SetName(string.format("%s·覺醒", pNpc.szName));
			end
		end
	end
end
