...
function tbHonorLevel:CheckFinishHonorLevel(pPlayer)
    ...
    if self.nMinOpenLevel > pPlayer.nLevel then
        return false, string.format("等級不足%s", self.nMinOpenLevel);
    end
    ...
    if tbHonorInfo.NeedPower > nFightPower then
        return false, string.format("目前戰力不足[FFFE0D]%d[-]，還需努力提升哦！", tbHonorInfo.NeedPower);
    end

    local nTotalStart = PersonalFuben:GetAllSectionStarAllLevel(pPlayer);
    if tbHonorInfo.NeedFubenStar > nTotalStart then
        return false, string.format("關卡總星級不足%d", tbHonorInfo.NeedFubenStar);
    end
...
function tbHonorLevel:CheckNeedItem(pPlayer, tbHonorInfo)
    ...
    if tbHonorInfo.ItemCount > nTotalCount then
        ...
        return false, string.format("本次晉升需[FFFE0D] %s [-]個[FFFE0D] %s [-]，快去收集吧！", tbHonorInfo.ItemCount, tbItemInfo.szName);
    end
...
function tbHonorLevel:FinishHonorLevel(pPlayer)
    ...
    if pPlayer.dwKinId ~= 0 then
        local szShowMsg = string.format("恭喜「%s」晉升為「%s」頭銜", pPlayer.szName, tbHonorInfo.Name);
    ...
    if tbHonorInfo.IsNotice == 1 then
        ...
        if nCurRank == 1 then
            szWorldMsg = string.format("%s全服第一晉升頭銜「%s」", pPlayer.szName, tbHonorInfo.Name);
        elseif nCurRank >= 2 and nCurRank <= 10 then
            szWorldMsg = string.format("%s全服前十晉升頭銜「%s」", pPlayer.szName, tbHonorInfo.Name);
        end
