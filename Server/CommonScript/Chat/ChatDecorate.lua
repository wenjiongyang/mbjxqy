...
function ChatDecorate:LoadSetting()
	...
	local tbThemeParams = {"nThemeID","nDefault","nSort","szTitle","szGet","szCondition","szParam","szGirlGet","szGirlTitle","version_tx","version_vn","version_hk","version_tw","version_xm","version_en","version_kor","version_th"};
...
function ChatDecorate:CheckVersion(tbInfo)
	if (tbInfo.version_tx == 1 and version_tx) or
		...
		(tbInfo.version_tw == 1 and version_tw) or
...
function ChatDecorate:CanApply(pPlayer,nPartsID)
	...
	if nCurTime < GetTime() then
		return false,XT(string.format("請先獲得%s主題,或該主題已經過期",self:GetTitleByThemeId(nThemeID,pPlayer.nFaction) or ""))
	end
