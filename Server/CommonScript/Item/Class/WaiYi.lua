...
function tbWaiyiEx:OnUse(it)
	...
	if nTargetId then
		...
		if tbBaseProp.nFactionLimit > 0 and me.nFaction ~= tbBaseProp.nFactionLimit then
			me.CenterMsg(string.format("該道具只能%s門派方可使用", Faction:GetName(tbBaseProp.nFactionLimit)))
			return 0;
		end
...
function tbWaiyiEx:UseConfirm(nItemId, bConfirm)
	...
	if not bConfirm and nTimeOut > 0 then
		me.MsgBox("使用後您將獲得一件[FFFE0D]限時外裝[-]並開始計算使用時間，請問是否使用?", {{"確定", self.UseConfirm, self, nItemId, true}, {"取消"}})
		return;
	end
