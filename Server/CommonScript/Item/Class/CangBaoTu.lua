...
tbItem.szDBZBlackMsg = "遭奪寶賊埋伏…";
tbItem.szBossBlackMsg = "不慎挖到野外首領的巢穴…";
...
function tbItem:OnUse(it)
	...
	if nItemType == self.TYPE_SENIOR then 
		if me.nLevel < nSeniorUseLevel then
			me.CenterMsg(string.format("少俠閱歷尚淺，需要等級達到%d級",nSeniorUseLevel))
			return
		end
	...
	GeneralProcess:StartProcessExt(me, self.tbSetting[nItemtype].nWaBaoTime * Env.GAME_FPS, true, 0, 0, "挖寶中", {self.OnEndProgress, self, it.dwId}, {self.OnBreakProgress, self, it.dwId});
	return 0;
end
...
function tbItem:CheckTeamMember(it,bCheckCommon)
	...
	if not me.dwTeamID or me.dwTeamID == 0 then
		return false,string.format("需%s人以上組隊前往挖寶",self.nNeedTeamate);
	end

	local tbMember = TeamMgr:GetMembers(me.dwTeamID)
	if not next(tbMember) then
		return false,"有隊友才能挖寶";
	end
