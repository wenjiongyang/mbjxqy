...
function tbItem:SendAward(szName, dwTemplateId, tbSetting)
	...
	if tbSetting.WorldMsg > 0 then
		...
		if nItemTemplateId then
			...
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.System, szMsg, nil, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemTemplateId, nFaction = me.nFaction});
		else
	...
	if tbSetting.KinMsg > 0 and me.dwKinId > 0 then
		...
		if nItemTemplateId then
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, me.dwKinId, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemTemplateId, nFaction = me.nFaction});
		else
	...
	if tbSetting.nNeedCount <= 0 then
		...
		for nItemId, nCount in pairs(tbSelect or {}) do
			...
			if pItem.nPos ~= Item.EITEMPOS_BAG then
				return false, "只能使用背包內的物品";
			end

			if pItem.szClass == "SkillBook" and not self.tbSkillBook:CheckCanSell(pItem) then
				return false, "升級後的秘笈不能使用";
			end
