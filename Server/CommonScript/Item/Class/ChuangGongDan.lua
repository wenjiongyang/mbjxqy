...
function tbItem:OnUse(it)
	...
	if nAddChuangGongSend > 0 then
		...
		szMsg = szMsg ..XT(string.format("增加了[FFFE0D]%d次[-]傳功次數", nAddChuangGongSend))
	end
	if nAddChuangGong > 0 then
		...
		szMsg = szMsg ..XT(string.format("增加了[FFFE0D]%d次[-]被傳功次數", nAddChuangGong))
	end
...
function tbItem:UseChuangGongDan(nItemId)
	...
	if nChuangGongSendDan > 0 and nChuangGongDan <= 0 then
		bTip = true
		szMsgTip = string.format("[FFFE0D]傳功丹所增加的傳功次數已達上限[-]\n使用後增加[FFFE0D]%d次[-]被傳功次數", nAddChuangGong)
	elseif nChuangGongDan > 0 and nChuangGongSendDan <= 0 then
		bTip = true
		szMsgTip = string.format("[FFFE0D]傳功丹所增加的被傳功次數已達上限[-]\n使用後增加[FFFE0D]%d次[-]傳功次數", nAddChuangGongSend)
	end
...
function tbItem:CheckUse(pPlayer)
	...
	if not bIsCross then
		return false, XT("少俠今日服用過傳功丹了（次日4:00可用）")
	end
	...
	if nChuangGongDan >= nAddChuangGongTimes and nChuangGongSendDan >= nAddChuangGongSendTimes then
		return false, XT("傳功丹所增加的傳功、被傳次數已達上限，請先消耗後再使用")
	end
