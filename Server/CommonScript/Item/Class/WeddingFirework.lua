...
local tbLegalMap = {10, 999, Kin.Def.nKinMapTemplateId}
tbItem.tbLegalMap = {}

for i,v in ipairs(tbLegalMap) do
    tbItem.tbLegalMap[v] = 1
end
...
function tbItem:OnUse(it)
	...
	if nPlayInterval < 3 then
		me.CenterMsg(string.format("[FFFE0D]%s秒[-]之後才可燃放", 3 - nPlayInterval > 0 and 3 - nPlayInterval or 0))
		return 
	end
...
function tbItem:IsWhiteMap(pPlayer)
	if self.tbLegalMap[pPlayer.nMapTemplateId] then
        return true;
    end
    ...
    return false, "只能在[FFFE0D]婚禮現場、忘憂島、襄陽城、幫派屬地、家園[-]燃放"
end
