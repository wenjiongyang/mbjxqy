...
function tbItem:OnUse(it)
	if not House:IsInOwnHouse(me) then
		me.CenterMsg("該道具只能在自己家園使用！請返回家園再嘗試！");
		return;
	end
...
function tbItem:OnClientUse()
	...
	if not House.bHasHouse then
		me.MsgBox("俠士目前還沒有家園，暫時無法使用該道具，何不前往[FFFE0D]真兒[-]處瞭解關於家園的消息？",
			{
				{"找真兒", function () Ui.HyperTextHandle:Handle("[url=npc:testtt, 2279, 10]", 0, 0); end},
				{"暫時不去"}
			});
		return 1;
	end
	...
	if not House:IsInOwnHouse(me) then
		me.MsgBox("該道具只能在自己家園使用！是否要返回家園？",
			{
				{"返回家園", function () RemoteServer.GoMyHome(); end},
				{"暫時不去"}
			});
		return 1;
	end
