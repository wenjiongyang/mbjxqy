...
function tbItem:GetTip(it)
	...
	return string.format([[等級：%s
舒適度：%s
傢俱類型：%s
擺放位置：%s]], tbFurniture.nLevel, tbFurniture.nComfortValue, Furniture:GetTypeName(tbFurniture.nType), szPutType);
end
...
function tbItem:GetUseSetting(nItemTemplateId, nItemId)
	if not House.bHasHouse then
		return {
					...
					fnFirst = function ()
								...
								me.MsgBox("你還沒有家園，傳聞[FFFE0D]真兒[-]處可打探到相關資訊。",
									{
										{"現在就去", function () Ui.HyperTextHandle:Handle("[url=npc:testtt,2279,10]", 0, 0); end},
										{"等會兒吧"}
									});
								end
				};
	...
	if not nItemId or nItemId <= 0 then
		if Ui:WindowVisible("HouseDecorationPanel") and not Ui:WindowVisible("HouseComfortableDetailsPanle") then
			local tbOpt = {};
			if not Furniture:IsSpecialFurniture(nItemTemplateId) then
			...
			if bCanPut then
				table.insert(tbOpt, { szName = "擺放", fnClick = function ( ... )
	...
	return {szFirstName = "放入傢俱倉庫", fnFirst = "UseItem"};
end
