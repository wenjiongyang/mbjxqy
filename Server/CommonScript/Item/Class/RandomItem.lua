...
function tbRandomItem:LoadItemList()
	...
	local tbTitle = {"ClassParamID", "Probability", "Name", "Item", "DebtReplaceItem", "OtherType", "SubType", "Amount","Add2Auction", "TimeLimit", "WorldMsg", "FriendMsg", "KinMsg", "TeamMsg"};
...
function tbRandomItem:AddAward(pPlayer, tbItem, szFromItemName, nLogReazon, nLogReazon2)
	...
	if tbItem.nItem and tbItem.nItem > 0 and tbItem.szOtherType == "" then
		...
		tbGetAward = {"item", tbItem.nItem, tbItem.nAmount, szTimeOut}
	...
	if pPlayer then
		...
		if tbItem.nWorldMsg > 0 then
			...
			if nItemId then
				...
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.System, szMsg, nil, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemId, nFaction = pPlayer.nFaction});
			else
				KPlayer.SendWorldNotify(0, 999, szMsg, 1, 1);
			end
		end

		if tbItem.nKinMsg > 0 and pPlayer.dwKinId > 0 then
			local szMsg = MsgInfoCtrl:GetMsg(tbItem.nKinMsg, pPlayer.szName, szFromItemName or "", szItemName);
			if nItemId then
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, pPlayer.dwKinId, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemId, nFaction = pPlayer.nFaction});
			else
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, pPlayer.dwKinId);
			end
		end
		if tbItem.nTeamMsg and tbItem.nTeamMsg > 0 and pPlayer.dwTeamID > 0 then
			local szMsg = MsgInfoCtrl:GetMsg(tbItem.nTeamMsg, pPlayer.szName, szFromItemName or "", szItemName);
			if nItemId then
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, szMsg, pPlayer.dwTeamID, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemId, nFaction = pPlayer.nFaction})
			else
...
function tbRandomItem:RandomItemAward(pPlayer, nKind, szFromItemName, nLogReazon, nLogReazon2)
	...
	for _, tbItem in ipairs(self.tbItemList[nKind].tbFixedItem) do
		...
		elseif pPlayer then
			Log(string.format(XT("%s隨機獲得物品失敗 箱子：%s，物品：%s"), pPlayer.szName, szFromItemName, tbItem.szName));
		end
	end
