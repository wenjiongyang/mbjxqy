...
function tbItem:OnUse(pItem)
	...
	if not nPartnerId then
		...
		me.CenterMsg(string.format("請先上陣同伴【%s】", szPName));
		return;
	end

	if tbPartner.nWeaponState ~= 0 then
		me.CenterMsg(string.format("上陣的【%s】已經擁有此武器", tbPartner.szName));
		return;
	end
	...
	me.MsgBox(string.format("確定裝備 [FFFE0D]%s[-] 本命武器 [FFFE0D]%s[-] ？\n[FFFE0D]（裝備後不可取下，遣散後將會返還）[-]", tbPartner.szName, szItemName),
		{
			{"確定", self.UseWeapon, self, me.dwID, nPartnerId, pItem.dwId},
			{"取消"}
		});
	return;
end
