...
{
    {
        nMin = 0,
        nMax = 10,
        szMsg = "使用後修煉珠增加[FFFE0D]30分鐘[-]野外修煉時間，修煉丹[FFFE0D]每天4點[-]恢復[FFFE0D]1次[-]使用次數，最多可以累積[FFFE0D]3次[-]";
    };

    {
        nMin = 11,
        nMax = 13,
        szMsg = "使用後修煉珠增加[FFFE0D]30分鐘[-]野外修煉時間，修煉丹[FFFE0D]每天4點[-]恢復[FFFE0D]1次[-]使用次數，最多可以累積[FFFE0D]3次[-]。\n升級尊14每天可使用次數增加[FFFE0D]1次[-]";
    };

    {
        nMin = 14,
        nMax = 30,
        szMsg = "使用後修煉珠增加[FFFE0D]30分鐘[-]野外修煉時間，修煉丹[FFFE0D]每天4點[-]恢復[FFFE0D]2次[-]使用次數，最多可以累積[FFFE0D]6次[-]";
    };
};
...
function tbItem:GetTip(pItem)
    ...
    local szTip = string.format("累積可使用次數：%s", nCount);
    return szTip;
end
...
function tbItem:GetShopTip(pItem)
    ...
    local nCount = self:GetOpenResidueCount(me);
    local szTip = string.format(szTipShow.."[FFFE0D]          累積可使用次數：%s[-]", nCount);
    return szTip;
end
...
function tbItem:CheckXiuLianResiduTime(pPlayer, nAddResiduTime)
    ...
    if nCount <= 0 then
        return false, "少俠剩餘可使用次數不足（次日4:00可用）";
    end
...
function tbItem:OnUse(it) 
    ...
    me.CenterMsg(string.format("累積修煉時間增加了%s分鐘[FFFE0D]（修煉珠）[-]", math.floor(nAddResiduTime / 60)));
    return 1;    
end