...
function tbXinDe:CheckUseXinDe(pPlayer, pItem)
    ...
    if pPlayer.nLevel < tbItemInfo.MinPlayerLevel then
        return false, string.format("少俠等級不足%s級，不可使用心得書", tbItemInfo.MinPlayerLevel);
    end

    if pPlayer.nLevel > tbItemInfo.PlayerLevel then
        return false, string.format("少俠等級大於%s級，不可使用該本心得書", tbItemInfo.PlayerLevel);
    end

    local nPlayerNextExp = pPlayer.GetNextLevelExp();
    local nPlayerExp = pPlayer.GetExp();
    if nPlayerExp >= nPlayerNextExp then
        return false, string.format("少俠當前經驗已滿，不可使用心得書");
    end

    local nCount = DegreeCtrl:GetDegree(pPlayer, "XinDeBook");
    if nCount <= 0 then
        return false, "今日可使用次數不足";
    end
...
function tbXinDe:GetTip(pItem)
    ...
    local szMsg = string.format("心得書等級：%s\n今日還可使用：%s次\n使用獲得經驗：%s", tbItemInfo.PlayerLevel, nCount, nGetExp);
    return szMsg;
end
...
function tbXinDe:GetUseSetting(nItemTemplateId, nItemId)
    ...
    return {szFirstName = "贈送", fnFirst = fnSong, szSecondName = "使用", fnSecond = "UseItem"};
end
...
function tbEmpty:GetTip(pItem)
    ...
    local szMsg = string.format("等級：%s\n修煉進度：%s%%", tbLevelInfo.PlayerLevel, math.floor(nSaveExp / nMaxXiuLianExp * 100));
    return szMsg;
end
