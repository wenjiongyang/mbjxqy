...
function Lib:GetAwardDesCount(tbAllAward, pPlayer)
	...
	for nIndex, tbAward in pairs(tbAllAward) do
		...
		elseif nAwardType == Player.award_type_kin_found then
			tbDes.szName = "幫派資金";
			tbDes.szDesc = string.format("幫派資金%s兩", tbAward[2]);
		...
		elseif nAwardType == Player.award_type_basic_exp then
			...
			tbDes.szDesc = string.format("經驗%s點", nCount);
		elseif nAwardType == Player.award_type_faction_honor then
			tbDes.szName = "門派榮譽";
			tbDes.szDesc = string.format("門派榮譽%s點", tbAward[2]);
		elseif nAwardType == Player.award_type_battle_honor then
			tbDes.szName = "戰場榮譽";
			tbDes.szDesc = string.format("戰場榮譽%s點", tbAward[2]);
		end
...
function Lib:GetCnTime(nHour)
	...
	if (nHour - nDigit == 0.5 and nDigit > 0) then
		szXiaoshi	= self:TransferDigit2CnNum(nDigit).."個半小時";
	elseif (nHour - nDigit == 0.5) then
		szXiaoshi	= "半個小時";
	else
		szXiaoshi	= self:TransferDigit2CnNum(nDigit).."個小時";
	end
...
function Lib:TimeDesc(nSec)
	...
	elseif (nSec < 3600 * 24) then	-- 小于1天
		return  string.format( nSec % 3600 == 0 and "%d小時" or "%.1f小時", nSec / 3600)
	else
...
function Lib:TimeDesc2(nSec)
	...
	elseif (nSec < nDaySec) then	-- 小于1天
		return string.format("%d小時%d分", nSec / 3600, (nSec % 3600) / 60);
	else
		local nSecLeft = nSec % nDaySec
		if nSecLeft==0 then
			return string.format("%d天", nSec / nDaySec);
		else
			return string.format("%d天%d小時", nSec / nDaySec, nSecLeft / 3600);
		end
	end
end
...
function Lib:TimeDesc5(nSec)
	...
	if nHour > 0 then
		if math.floor(nHour / 24) > 0 then
			return string.format("%d天%d小時%d分",math.floor(nHour / 24),nHour % 24 ,nMin);
		else
			return string.format("%d小時%d分",nHour,nMin);
		end
	end
...
function Lib:TimeDesc6(nSec)
	...
	elseif (nSec < 3600) then	-- 小于1小时
		return string.format("%d分%d秒", nSec / 60, math.mod(nSec, 60))
	elseif (nSec < 3600 * 24) then	-- 小于1天
		return  string.format("%d小時", math.floor(nSec / 3600))
	else
...
function Lib:TimeDesc7(nTime)
	...
	return string.format("%s年%s月%s日%s點",tbTime.year,tbTime.month,tbTime.day,tbTime.hour)
...
function Lib:TimeDesc9(nTime)
	nTime = math.max(0, nTime)
	local tbTime    = os.date("*t", nTime);
	return string.format("%s年%s月%s日%s點%s分%s秒",tbTime.year,tbTime.month,tbTime.day,tbTime.hour,tbTime.min,tbTime.sec)
end

function Lib:TimeDesc10(nTime)
	nTime = math.max(0, nTime)
	local tbTime    = os.date("*t", nTime);
	return string.format("%s月%s日%s點",tbTime.month,tbTime.day,tbTime.hour)
end
...
function Lib:TimeDesc12(nSec)
	...
	elseif (nSec < 3600) then	-- 小于1小时
		return string.format("%d分%d秒", nSec / 60, math.mod(nSec, 60))
	elseif (nSec < 3600 * 24) then	-- 小于1天
		return  string.format("%d時%d分%d秒", math.floor(nSec / 3600), math.mod(nSec, 3600) / 60, math.mod(math.mod(nSec, 3600), 60))
	else
		return string.format("%d天", math.floor(nSec/(3600 * 24)))
	end
end
...
function Lib:TimeDesc15(nSec)
	...
	elseif (nSec < nDaySec) then	-- 小于1天
		...
		if nHourLeft==0 then
			...
			return string.format("%d天%d小時", nSec / nDaySec, nHourLeft);
		end
	end
end
...
function Lib:ParseDateTime(szDateTime)
	...
	if not year then
		Log("Lib:ParseDateTime 時間字串格式不合法" .. szDateTime);
		return;
	end
