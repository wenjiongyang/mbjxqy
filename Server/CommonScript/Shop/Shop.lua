...
function Shop:LoadWares()
    local tbWares = LoadTabFile(
        "Setting/Shop/Wares.tab", "dddssddssddddddss",nil,
        {"GoodsId", "TemplateId", "Price", "MoneyType", "ShopType", "Sort", "Discount", "TimeFrame", "CloseTimeFrame", "LimitType", "LimitNum", "HotTip", "MinLevel", "ShowLevel", "ShowVipLevel", "OpenTime", "CloseTime"});
...
Shop.tbMarketable =  LoadTabFile("Setting/Shop/MarketableThing.tab",
    "ddsd", "TemplateId",
    {"TemplateId", "Price", "MoneyType", "PlayerLevel"});
...
function Shop:_CanBuyWare(pPlayer, tbWare, nBuyCount)
    ...
    if tbWare.nMinLevel and tbWare.nMinLevel > pPlayer.nLevel then
        return false, string.format("角色%s級後可購買", tbWare.nMinLevel)
    end
...
function Shop:CheckWareTimeFrame(tbWare, szTimeFrame)
    if tbWare.nLimitType == self.WEEK_LIMIT_TYPE then
        ...
        local nLastWeekStartTime = Lib:GetLocalWeekEndTime() - 3600*24*7 + Shop.TREASURE_REFRESH
...
function Shop:LoadFamilyPool()
    local tbPoolSetting = LoadTabFile(
        "Setting/Shop/FamilyPool.tab", "ddsddddd", nil,
        {"Level", "PlayerLevel","ShopType", "Pool", "Count1", "Prob1", "Count2", "Prob2"});
    ...
    for _,v in pairs(tbPoolSetting) do
        ...
        tbPool[v.Level][v.Pool] = {v.Count1, v.Prob1, v.Count2, v.Prob2};
...
function Shop:LoadFamilyWares()
    local tbWares = LoadTabFile(
        "Setting/Shop/FamilyWares.tab", "ddssdddddsdd",nil,
        {"TemplateId", "Price", "MoneyType", "ShopType", "Sort", "Level", "Pool", "Count", "Discount", "TimeFrame", "HotTip", "MinLevel"});
...
function Shop:LoadFamilyDiscount()
    -- tab 中的level 是差的等级＋１
    local tbDiscountSetting = LoadTabFile(
        "Setting/Shop/FamilyDiscount.tab", "sdddddddddd", "ShopType",
        {"ShopType", "Level1", "Level2","Level3","Level4","Level5","Level6","Level7","Level8","Level9", "Level10"});
