...
function OnHook:CheckCommond(pPlayer)
	...
	if not self:IsOpen(pPlayer) then
		return false,"離線託管尚未開放";
	end

	local nExpTime = self:ExpTime(pPlayer);
	if nExpTime < 60 then													-- 离线时间按分钟换算经验,所以不能小于60秒
		return false,"沒有可領取的離線經驗"
	end
...
function OnHook:PayTips(pPlayer,nPayType)
	...
	if nExpTime > nBaiJuWanTime then
		...
		if nHaveNum > 0 then
			szTip = szTip ..string.format("已擁有[FFFE0D]%d個[-]%s",nHaveNum,KItem.GetItemShowInfo(nItemId, pPlayer.nFaction))
		end
		...
		if nLackNum > 0 then																	-- 需要用元宝的情况
			...
			local nNeedPay = nLackNum * nPerBaiJuWanPrice;
			 ...
			 local szNeedBuy = string.format("需再購買[FFFE0D]%d個[-]",nLackNum)
			 if nHaveNum <= 0 then
			 	szNeedBuy = szNeedBuy .. KItem.GetItemShowInfo(nItemId, pPlayer.nFaction)
			 end
			 local szNeedPay = string.format("，花費[FFFE0D]%d[-]%s",nNeedPay,szMoneyEmotion);
...
function OnHook:CheckRequestOnlineOH(pPlayer)
	if pPlayer.nRequestStartOnlineOHTime and GetTime() < pPlayer.nRequestStartOnlineOHTime then
		...
		return false,string.format("託管操作太頻繁了,請%d秒之後再嘗試",nSecond)
	end
