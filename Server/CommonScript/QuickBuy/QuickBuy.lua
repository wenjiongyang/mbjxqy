...
local tbSettings = 
{
	PrayTimes = 
	{
		...
		Enable = function (pPlayer)
			...
			return nDegree > 0, "今日的祈福購買次數已經用完";
		end,		
...
function QuickBuy:NotifyQuickBuy(pPlayer, szBuyType, tbParams)
    ...
    local szMsg = string.format("%s(花費%d%s) \n今日剩餘可購買%d次", szTip, nMoney, szMoneyName, tbSetting.GetLeftTimes(pPlayer) );
