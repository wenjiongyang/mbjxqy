...
MarketStall.nOpenLevel = 20					-- 摆摊功能开启等级
...
MarketStall.nMaxRandomRefreshTime = 40 * 60 -- 上架全服摊位，最大随机时间
...
MarketStall.bOpenMaxPriceLimitDynamic = true;  --摆摊基准价最高倍数随时间轴变动，开关

MarketStall.nAvaragePriceCount = 10;		-- 推荐价格是最近几次的价格平均值

MarketStall.nSellLimitTime = 7*24*3600;	-- 卖出CD
...
if version_tx then
	...
	MarketStall.bSellLimitOpen = true;			-- 卖出CD功能开关
end
...
function MarketStall:Load()
	...
	local tbParam = {"szMainType", "nSubType", "nSellLimitType", "nSellLimitSubType", "nSellLimitLevel", "nType", "nIndex", "nMinPrecent", "nMaxPrecent", "szOpenTimeFrame", "szCloseTimeFrame", "nPrice", "nShowLimitPrecent"};
	...
	self.tbAllType = LoadTabFile("Setting/MarketStall/MarketTypeDef.tab", "ddsd", "nTypeId", {"nTypeId", "nShowCount", "szTypeName", "nIndex"});
...
function MarketStall:IsMarketOpen(pPlayer)
	if GetTimeFrameState(self.szCreateRoleTimeLimitTimeFrame) == 1 and GetTime() - pPlayer.dwCreateTime < self.nCreateRoleTimeLimit then
		return false, "新建角色24小時後開啟擺攤";
	end

	if pPlayer.nLevel < self.nOpenLevel then
		return false, string.format("%d級後開放擺攤", self.nOpenLevel)
	end
...
function MarketStall:CheckCanSellItem(pPlayer, szMainType, nSubType)
	...
	if nAwardType == Player.award_type_item then
		...
		if tbInfo.szCloseTimeFrame and tbInfo.szCloseTimeFrame ~= "" and GetTimeFrameState(tbInfo.szCloseTimeFrame) == 1 then
			return false, "陳舊物品，不能上架";
		end

		if pItem.nPos ~= Item.emITEMPOS_BAG then
			return false, "只有背包內的道具才可以擺攤";
		end
...
function MarketStall:CheckCanNewSellItem(pPlayer, szMainType, nSubType, nCount, nPrice, bIgnorCheckCost)
	...
	if not bCanSell then
		return false, string.format("%s後才可以上架此物品", Lib:TimeDesc6(nLastTime));
	end

	if not bIgnorCheckCost and nCurCount < nCount then
		return false, "大俠沒有這麼多物品呀！";
	end

	if not tbPriceInfo or not tbPriceInfo[nPrice] then
		return false, "這東西不能使用這個價格上架喔！";
	end
...
function MarketStall:CheckCanUpdateItemPrice(pPlayer, szMainType, nSubType, nCount, nPrice, bIgnorCheckCost)
	...
	if not bCanSell then
		return false, string.format("%s後才可以重新上架此物品", Lib:TimeDesc6(nLastTime));
	end
...
function MarketStall:TipBuyItemFromMarket(pPlayer, nItemId, szTipsInfo)
	...
	if not szTipsInfo then
		...
		szTipsInfo = string.format("[FFFE0D]%s[-]數量不足，是否前往擺攤購買？", szItemName);
	end
