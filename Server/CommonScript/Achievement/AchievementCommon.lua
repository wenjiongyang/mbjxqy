...
function Achievement:LoadSetting()
    ...
    for _, tbInfo in ipairs(tbSetting) do
        ...
        self.tbMainSetting[tbInfo.Kind] = { nKeyID = tbInfo.KeyID, nMaxLevel = 0, tbSubKind = {}, tbLevelInfo = {}, nGroup = tbInfo.UiGroupID }
    end

    local tbKey = { Level = 1, FinishCount = 1, AwardTemplateId = 1, AwardCount = 1, AwardTitle = 1, NeedPush = 1, ListType = 1 }
    ...
    for _, tbInfo in ipairs(tbLevelSetting) do
        ...
        self.tbSubKindSetting[szSubKind] = self.tbSubKindSetting[szSubKind] or { szMainKind = tbInfo.ParentKind, tbCount = {} }
        table.insert(self.tbSubKindSetting[szSubKind].tbCount, {tbInfo.Level, tbInfo.FinishCount})

        local tbMain = self.tbMainSetting[tbInfo.ParentKind]
        tbInfo.tbAward = Lib:GetAwardFromString(tbInfo.Award)
        if tbInfo.AwardTitle > 0 then
            table.insert(tbInfo.tbAward, {"AddTimeTitle", tbInfo.AwardTitle, -1})
        end
...
function Achievement:GetPushMsg(szMainKind, nLevel)
    ...
    if tbLevelInfo.NeedPush == 1 then
        local szMsg = string.format("達成了<成就：%s>", tbLevelInfo.Title)
        return szMsg
    end
end
