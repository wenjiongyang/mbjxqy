...
function PartnerHead:SetPartnerById(nPartnerTemplateId, nLevel)
  local szName, nQualityLevel, nNpcTemplateId = GetOnePartnerBaseInfo(nPartnerTemplateId)
  self:SetPartnerFace(nNpcTemplateId, nQualityLevel)
end
function PartnerHead:SetPartnerInfo(tbPartner)
  self:SetPartnerFace(tbPartner.nNpcTemplateId, tbPartner.nQualityLevel, tbPartner.nLevel, tbPartner.nFightPower)
end
function PartnerHead:SetPartnerFace(nNpcTemplateId, nQualityLevel, nLevel, nFightPower)
  ...
  self.pPanel:SetActive("LightAnimation", tbShowInfo[3] and true or false)
  if tbShowInfo[3] then
