...
function tbUi:SetGradeLevel(szType)
  self.tbAllType = self.tbAllType or {
    "C",
    "B",
    "A",
    "S",
...
function tbUi:DoJoinKin(szButtonName)
  ...
  if dwKinId > 0 and 0 >= me.dwKinId then
    RemoteServer.OnKinRequest("Apply", dwKinId)
  elseif dwKinId <= 0 and 0 < me.dwKinId then
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_UPDATE_FUBEN_SCROE,
      self.UpdateScroe,
      self
    }
  }
