...
function tbUi:Update(szSubType)
  ...
  local bShowJingMaiRedPoint = JingMai:CheckShowRedPoint()
...
function tbUi:SubUpdateQualitySkill(tbPartnerInfo, tbPSkillInfo, tbPAttribInfo)
  ...
  for _, szType in ipairs(self.tbProtentialList) do
    ...
    local nLimitProtential, bIsMaxGrade = Partner:GetLimitProtentialValue(tbPartnerInfo.nQualityLevel, tbPartnerInfo.nGrowthType, Partner.tbAllProtentialTypeStr2Id[szType], nLimitLevel, tbPartnerInfo.nGradeLevel + 1, tbPartnerInfo.nAwareness)
  ...
  for i = 1, 3 do
    if tbPSkillInfo.tbDefaultSkill[i] then
      table.insert(tbSkillInfo, {
        tbPSkillInfo.tbDefaultSkill[i].nSkillId,
        tbPSkillInfo.tbDefaultSkill[i].nSkillLevel
      })
    end
  end
  for i = 1, 5 do
    if tbPSkillInfo.tbNormalSkill[i] then
      table.insert(tbSkillInfo, {
        tbPSkillInfo.tbNormalSkill[i].nSkillId,
        tbPSkillInfo.tbNormalSkill[i].nSkillLevel
      })
    end
  end
...
function tbUi:SubUpdateAttribInfoPanel(tbPartnerInfo, tbPSkillInfo, tbPAttribInfo)
  ...
  for nIdx, tbInfo in ipairs(Partner.tbAllAttribDef) do
    ...
    if not self.pPanel:CheckHasChildren("AttribInfo" .. nIdx) then
      ...
      self.pPanel:ChangePosition("AttribInfo" .. nIdx, 10, 245 - 40 * nIdx - #Partner.tbAllAttribDef * 2)
...
function tbUi:SetSkill(tbSkillInfo)
  ...
  for i = 2, 8 do
    ...
    if tbValue then
      ...
      if not self.UI_NAME and i >= 4 then
...
function tbUi:OnPartnerGradeLevelup(nPartnerId, nOldGradeLevel, nNewGradeLevel)
  ...
  for i, szType in ipairs(self.tbProtentialList) do
    local tbValue = {}
    local nValue = tbPAttribInfo["n" .. szType]
    local nLimitProtential = Partner:GetLimitProtentialValue(tbPartnerInfo.nQualityLevel, tbPartnerInfo.nGrowthType, Partner.tbAllProtentialTypeStr2Id[szType], tbPartnerInfo["nLimitProtential" .. szType], tbPartnerInfo.nGradeLevel + 1, Partner:GetPartnerAwareness(me, tbPartnerInfo.nTemplateId))
    local nMaxValue = tbPAttribInfo["n" .. szType] + math.max(nLimitProtential - math.floor(tbPartnerInfo["nProtential" .. szType] / Partner.tbProtentialToValue[tbPartnerInfo.nQualityLevel]), 0)
...
function tbUi:OnClickSkill(nIdx)
  ...
  if not nIdx or self.UI_NAME then
    Partner:ShowSkillTips(nSkillId, nSkillLevel, nMaxLevel)
  else
  ...
  me.MsgBox(string.format("无可用技能书，是否前往摆摊购买？"), {
    {
      "前往",
      function()
        Ui:OpenWindow("MarketStallPanel", 1)
      end
    },
    {"取消"}
  })
...
function tbUi:ExchangePartnerPos(nPos1, nPos2)
  ...
  if not (nPos1 and nPos2) or me.nLevel < Partner.tbPosNeedLevel[nPos1] or me.nLevel < Partner.tbPosNeedLevel[nPos2] then
    return
  end
...
function tbUi.tbOnDrag:PartnerView(szWnd, nX, nY)
...
function tbUi.tbOnClick:BtnCompanionArray()
  if not self.nCurPartnerId or self.nCurPartnerId <= 0 then
    me.CenterMsg("当前没有同伴")
    return
  end
...
function tbUi.tbOnClick:BtnSeverance()
...
function tbUi.tbOnClick:BtnAwareness()
  Ui:OpenWindow("PartnerAwarenessPanel", self.nCurPartnerId)
end
function tbUi.tbOnClick:Weapon()
...
function tbUi.tbOnClick:BtnMeridian()
...
function tbUi.tbOnClick:Meridian()
  ...
  Ui:OpenWindow("JingMaiTipsPanel", tbAddInfo.tbExtPartnerAttrib, tbAddInfo.tbPartnerSkill)
end
function tbUi.tbOnClick:Dantian()
  local nDanTianValue = me.GetUserValue(JingMai.SAVE_GROUP_ID, JingMai.SAVE_INDEX_ID)
