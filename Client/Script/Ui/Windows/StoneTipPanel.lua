...
function tbUi:SetItem(nItemId, nTemplateId)
  ...
  for nIndex, nTId in ipairs(tbTemplateId) do
    ...
    local szName, nIcon, nView, nQuality = KItem.GetItemShowInfo(nTId, me.nFaction)
    local szNameColor = Item:GetQualityColor(nQuality) or "White"
    if nIcon then
    ...
    itemObj.pPanel:Label_SetColorByName("Number" .. nIndex, szNameColor)
  end
  if nMyIndex then
