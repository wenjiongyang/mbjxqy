...
function tbUi:GetViewItemIds()
  ...
  if self.szType == "Enhance" then
    ...
    for i, nEquipPos in ipairs(tbEquipPos) do
      ...
      if nStrengthLevel < tbCanEnhacne.nMaxEnhance and (not tbCanEnhacne.nMinEnhance or nStrengthLevel >= tbCanEnhacne.nMinEnhance) then
...
function tbUi:UpdateItemList(tbViewItemIds)
  local function fnClickItem(goodItem)
  ...
  local function fnSetItem(itemObj, nIndex)
    for i = 1, 2 do
      ...
      if nItemId then
        ...
        local szItemName, nIcon, nView = pItem.GetItemShowInfo(me.nFaction)
        if self.szType == "Enhance" then
