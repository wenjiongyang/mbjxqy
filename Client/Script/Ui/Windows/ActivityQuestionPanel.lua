...
function tbUi:RegisterEvent()
  return {
    ...
    {
      UiNotify.emNOTIFY_DATI_DATA_CHANGE,
      self.Update,
      self
    }
  }
...
function tbUi:Update(szType, nQuestionId, ...)
  ...
  if szType == "ZhongQiuJie" or szType == "WeekendQuestion" then
    ...
    self:CloseZQJ_Timer()
    self.nZQJ_Timer = Timer:Register(Env.GAME_FPS, function()
...
function tbUi:UpdateQuestion()
  self:UpdateDetail()
  self:UpdateAward()
  local nNpcTemplateId = self.tbCurQues.nNpcTemplateId or 75
...
function tbUi:UpdateDetail()
  ...
  elseif self.szType == "Activity" then
    ...
    local tbAward = {
      ActivityQuestion.tbRightAward,
      ActivityQuestion.tbWrongAward,
...
function tbUi:UpdateAward()
  ...
  if self.szType == "ZhongQiuJie" then
    tbAward = {
      Activity.ZhongQiuJie.tbQuestionAward_Right,
      Activity.ZhongQiuJie.tbQuestionAward_Wrong
    }
  elseif self.szType == "Activity" then
    tbAward = {
      ActivityQuestion.tbRightAward,
  ...
  elseif self.szType == "WeekendQuestion" then
    tbAward = {
      Activity.WeekendQuestion.RIGHT_AWARD[1],
      Activity.WeekendQuestion.WRONG_AWARD[1]
    }
  end
...
function tbUi:AnswerQuestion(nIdx)
  ...
  elseif self.szType == "ZhongQiuJie" or self.szType == "WeekendQuestion" then
