...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    ...
    {
      UiNotify.emNOTIFY_CHAT_DEL_PRIVATE,
      self.OnDelPrivateMsg
    },
    {
    ...
    {
      UiNotify.emNOTIFY_VOICE_PLAY_START,
      self.OnVoiceStart
    },
    {
      UiNotify.emNOTIFY_VOICE_PLAY_END,
      self.OnVoiceEnd
    },
    {
      UiNotify.emNOTIFY_DYN_CHANNEL_CHANGE,
      self.OnDynChannelChange
    },
    {
      UiNotify.emNOTIFY_CHAT_CROSS_HOST,
      self.OnCrossInfoChanged
    },
    {
...
function tbUi:OnOpen(nChannelId, nFriendId, szParam)
  self.pPanel:SetActive("BtnClose", not InDifferBattle.bRegistNotofy or InDifferBattle.nState ~= 1)
  ...
  if self.nNowChatFriendId and (not ChatMgr.RecentPrivateList[1] or ChatMgr.RecentPrivateList[1].dwID ~= self.nNowChatFriendId) then
...
function tbUi:OnOpenEnd(nChannelId, nFriendId, szParam)
  ...
  elseif szParam == "OpenEmotionLink" then
    self.ChatInput.tbOnClick.BtnTypeAndLink(self.ChatInput)
  end
...
function tbUi:OnScreenClick(szTouchUi)
  if szTouchUi == "ChatSmall" or szTouchUi == "DreamlandPanel" then
    return
  end
...
function tbUi:UpdateNew()
  for i = 1, nChannelMaxCount do
    ...
    if nChannelId then
      local btnObj = self["BtnChannel" .. i]
      if nChannelId == ChatMgr.ChannelType.Public or nChannelId == ChatMgr.ChannelType.Nearby or nChannelId == ChatMgr.ChannelType.System or nChannelId >= ChatMgr.nDynChannelBegin then
        btnObj.pPanel:SetActive("New", false)
      elseif nChannelId ~= ChatMgr.nChannelMail and nChannelId ~= ChatMgr.ChannelType.Private and nChannelId ~= ChatMgr.nChannelFriendName then
...
function tbUi:NewChatMsg(nChannelId, tbMsg)
  ...
  if self:IsBrowsingHistory() and tbMsg.nSenderId ~= me.dwID then
...
function tbUi:SwitchChannel(nChannelId)
  if self.tbChannel[#self.tbChannel] == ChatMgr.nChannelBlackList and nChannelId ~= ChatMgr.nChannelBlackList then
...
function tbUi:UpdateBlackList()
  ...
  local function fnDelete(btnObj)
  ...
  local function fnSetItem(itemObj, nIndex)
    ...
    local szPortrait, szAtlas = PlayerPortrait:GetSmallIcon(tbItem.nPortrait or tbItem.nFaction)
...
function tbUi:UpdateChatList(bGoBottom)
  ...
  if tbLastMsg.nSenderId == me.dwID or self[szChatScrollViewName]:IsBottom() then
    bGoBottom = true
  end
  self[szChatScrollViewName]:UpdateItemHeight({40})
...
function tbUi:ShowMails()
  ...
  local nNow = GetTime()
  local function fnTimeDesc(nTime)
...
function tbUi:UpdateFriendChatBtn()
  ...
  if self.nNowChatFriendId and not FriendShip:IsHeInMyBlack(self.nNowChatFriendId) and self.nChannelId ~= ChatMgr.nChannelBlackList then
...
function tbUi:AddTeamLink()
  ...
  ChatMgr:SetChatLink(ChatMgr.LinkType.Team, {
    me.dwID,
    TeamMgr:GetTeamId(),
...
function tbUi:OnVoiceStart(uFileIdHigh, uFileIdLow, szApolloVoiceId)
  for i = 0, 1000 do
    ...
    if itemObj then
      if itemObj.tbMsg and (uFileIdHigh ~= 0 and uFileIdLow ~= 0 and itemObj.tbMsg.uFileIdHigh == uFileIdHigh and itemObj.tbMsg.uFileIdLow == uFileIdLow or itemObj.tbMsg.szApolloVoiceId and itemObj.tbMsg.szApolloVoiceId == szApolloVoiceId) then
        if itemObj.SelfMsg.pPanel:IsActive("Main") then
          itemObj.SelfMsg.pPanel:Sprite_Animation("Speaker", "PlayingVoice", nil, 4)
          break
        end
        if itemObj.OtherMsg.pPanel:IsActive("Main") then
          itemObj.OtherMsg.pPanel:Sprite_Animation("Speaker", "PlayingVoice", nil, 4)
        end
        break
      end
    else
...
function tbUi:OnVoiceEnd(uFileIdHigh, uFileIdLow, szApolloVoiceId)
  for i = 0, 1000 do
    ...
    if itemObj then
      if itemObj.tbMsg and (uFileIdHigh ~= 0 and uFileIdLow ~= 0 and itemObj.tbMsg.uFileIdHigh == uFileIdHigh and itemObj.tbMsg.uFileIdLow == uFileIdLow or itemObj.tbMsg.szApolloVoiceId and itemObj.tbMsg.szApolloVoiceId == szApolloVoiceId) then
        if itemObj.SelfMsg.pPanel:IsActive("Main") then
          itemObj.SelfMsg.pPanel:Sprite_Animation("Speaker", "PlayingVoice4", nil, 1)
          break
        end
        if itemObj.OtherMsg.pPanel:IsActive("Main") then
          itemObj.OtherMsg.pPanel:Sprite_Animation("Speaker", "PlayingVoice4", nil, 1)
        end
        break
      end
    else
...
function tbUi.tbOnClick:BtnAnchor()
  ...
  elseif ChatMgr:IsCrossHost(me) then
...
function tbItemUi:Init(tbMsg, nChannelId)
  ...
  self.tbMsg = tbMsg
  self:DealLink()
  local szMsg = string.format("[365888]%s[-]", tbMsg.szShowMsg or tbMsg.szMsg)
  self.pPanel:Label_SetText("TxtChatMsg", szMsg)
  self.pPanel:Label_SetText("Level", tbMsg.nLevel or "")
  local voiceSize = {x = 0, y = 0}
  if tbMsg.uFileIdHigh and 0 < tbMsg.uFileIdHigh and tbMsg.uFileIdLow and 0 < tbMsg.uFileIdLow or tbMsg.szApolloVoiceId and tbMsg.szApolloVoiceId ~= "" then
    self.pPanel:SetActive("VoiceNode", true)
    self.pPanel:Sprite_Animation("Speaker", "PlayingVoice4")
    self.pPanel:Label_SetText("TxtVoiceTime", Lib:TimeDesc4(math.floor(tbMsg.uVoiceTime / 1000)))
  ...
  local szHead, szAtlas = PlayerPortrait:GetSmallIcon(tbMsg.nPortrait or tbMsg.nFaction)
  self.pPanel:Sprite_SetSprite("Head", szHead, szAtlas)
  local szNamePrefix = ChatMgr:GetNamePrefix(tbMsg.nNamePrefix, false, nChannelId, tbMsg.nFaction, tbMsg.nSenderId)
  ...
  if not bDefaultHeadFrame then
    local nHeadFrameIcon = not bDefaultHeadFrame and ChatDecorate:GetIcon(tbMsg.nFaction, nHeadFrame) or ChatDecorate.Default[ChatDecorate.PartsType.HEAD_FRAME].nIcon
  ...
  local nBubbleIcon = not bDefaultBubble and ChatDecorate:GetIcon(tbMsg.nFaction, nBubble) or ChatDecorate.Default[ChatDecorate.PartsType.BUBBLE].nIcon
  ...
  if nChannelId == ChatMgr.ChannelType.Kin and not bSelf then
    ...
    if bLeader then
      szKinTitle = "领袖"
    elseif nCareer and Kin.Def.tbManagerCareers[nCareer] then
...
function tbItemUi:DealLink()
  ...
  function self.TxtChatMsg.pPanel.OnTouchEvent(tbObj, id)
...
function tbItemUi.InitSystemMsg(itemObj, tbMsg)
  ...
  if tbLinkInfo and tbLinkInfo.nLinkType and ChatMgr.tbLinkClickFns[tbLinkInfo.nLinkType] then
    function itemObj.TxtSystemMsg.pPanel.OnTouchEvent(tbObj, id)
      ChatMgr.tbLinkClickFns[tbLinkInfo.nLinkType](tbLinkInfo)
    end
  end
...
function tbItemUi.tbOnClick:Head(_, nPosX, nPosY)
  if self.tbMsg.nSenderId == me.dwID or self.tbMsg.nSenderId <= 0 then
    return
  end
  local dwKinId = 0
