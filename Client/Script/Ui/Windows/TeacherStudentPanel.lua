...
tbUI.tbOnClick = {
  BtnCancel = function(self)
    ...
    me.MsgBox(string.format("确定取消与[FFFE0D]%s[-]解除师徒关系的申请吗？", tbOtherMainInfo.szName), {
      {"确定", fnConfirm},
      {"取消"}
    })
  end,
  ...
  BtnApprenticeship = function(self)
    local nNpcTemplateId = 1839
    local nMapTemplateId = 999
...
function tbUI:RefreshFindTeacher()
  ...
  self.pPanel:Label_SetText("TipFindMaster", "暂无可收徒的侠士")
  self.pPanel:SetActive("TipFindMaster", true)
  self.pPanel:SetActive("BtnViewRules", false)
  local bCan = self:_CanShowTab(self.szTab)
  if not bCan then
    self.pPanel:Label_SetText("TipFindMaster", "少侠不符合拜师条件")
    self.pPanel:SetActive("BtnViewRules", true)
  end
  local tbTeacherList = bCan and TeacherStudent:GetTeacherList() or {}
  self.pPanel:SetActive("TipFindMaster", #tbTeacherList <= 0)
  self.FindMasterScrollView:Update(#tbTeacherList, function(pGrid, nIdx)
    ...
    local tbHonorInfo = Player.tbHonorLevelSetting[tbTeacher.nHonorLevel]
    local bHonorValid = tbHonorInfo and tbHonorInfo.ImgPrefix
    if bHonorValid then
      pGrid.pPanel:Sprite_Animation("PlayerTitle", tbHonorInfo.ImgPrefix)
    end
    pGrid.pPanel:SetActive("PlayerTitle", bHonorValid)
    ...
    function pGrid.Btn.pPanel.OnTouchEvent()
      TeacherStudent:ApplyAsStudent(tbTeacher.nId)
    end
    function pGrid.BtnDetails.pPanel.OnTouchEvent()
      self:ShowDetailMenu(tbTeacher.nId)
    end
    function pGrid.Head.pPanel.OnTouchEvent()
      self:ShowDetailMenu(tbTeacher.nId)
    end
...
function tbUI:ShowDetailMenu(nId)
  local tbData = ChatMgr:GetFriendOrPrivatePlayerData(nId)
  local dwKinId = tbData.dwKinId or 0
  local nLevel = tbData.nLevel or 0
  Ui:OpenWindowAtPos("RightPopup", 155, -125, "RoleSelect", {
    dwRoleId = nId,
    bFromChatList = false,
    dwKinId = dwKinId,
    nLevel = nLevel
...
function tbUI:_RefreshTeacherSettings()
  ...
  function self.NoUse.pPanel.OnTouchEvent()
    TeacherStudent:ChangeTeacherClosed(true)
  end
  function self.Find.pPanel.OnTouchEvent()
...
function tbUI:RefreshFindStudent()
  ...
  self.pPanel:Label_SetText("NoApprentice", "暂无可拜师的侠士")
  self.pPanel:SetActive("NoApprentice", true)
  self.pPanel:SetActive("BtnViewRules2", false)
  local bCan = self:_CanShowTab(self.szTab)
  if not bCan then
    self.pPanel:Label_SetText("NoApprentice", "少侠不符合收徒条件")
    self.pPanel:SetActive("BtnViewRules2", true)
  end
  local tbStudentList = bCan and TeacherStudent:GetStudentList() or {}
  self.pPanel:SetActive("NoApprentice", #tbStudentList <= 0)
  self.FindApprenticeScrollView:Update(#tbStudentList, function(pGrid, nIdx)
    ...
    local tbHonorInfo = Player.tbHonorLevelSetting[tbStudent.nHonorLevel]
    local bHonorValid = tbHonorInfo and tbHonorInfo.ImgPrefix
    if bHonorValid then
      pGrid.pPanel:Sprite_Animation("PlayerTitle", tbHonorInfo.ImgPrefix)
    end
...
function tbUI:GetOfflineDesc(nLastOnlineTime)
  ...
  elseif nSec < 604800 then
    return string.format("%d天前", math.floor(nSec / 86400))
  end
  return "7天前"
end
function tbUI:_MainInfoRefreshTeachers(tbTeachers)
  ...
  for i, tbTeacher in ipairs(tbSortedTeachers) do
    ...
    pMasterItem.pPanel:Label_SetText("OnLine", string.format("%s", 0 >= tbTeacher.nLastOnlineTime and "状态：" .. szOnline or "离线：" .. szOnline))
    local bOffline = 0 < tbTeacher.nLastOnlineTime
    ...
    local tbHonorInfo = Player.tbHonorLevelSetting[tbTeacher.nHonorLevel]
    local bTitleValid = tbHonorInfo and tbHonorInfo.ImgPrefix
    if bTitleValid then
      pMasterItem.pPanel:Sprite_Animation("PlayerTitle", tbHonorInfo.ImgPrefix)
    end
    pMasterItem.pPanel:SetActive("PlayerTitle", bTitleValid)
    self.pPanel:SetActive(szMasterItemName, true)
    function pMasterItem.pPanel.OnTouchEvent()
      self:_SelectPlayer(tbTeacher.nId)
      Ui:ClearRedPointNotify(string.format("TS_Teacher%d", i))
    end
    function pMasterItem.BtnChat.pPanel.OnTouchEvent()
      self:_PrivateChat(tbTeacher.nId)
    end
    function pMasterItem.itemframe.pPanel.OnTouchEvent()
      self:ShowDetailMenu(tbTeacher.nId)
    end
  end
end
...
function tbUI:_SortStudents(tbOrg)
  local tbRet = {}
...
function tbUI:_MainInfoRefreshStudents(tbStudents)
  ...
  local tbSortedStudents = self:_SortStudents(tbStudents)
  ...
  self.ApprenticeScrollView:Update(#tbSortedStudents, function(pGrid, nIdx)
    ...
    pGrid.pPanel:SetActive("PlayerTitle", bTitleValid)
    ...
    if tbStudent.bGraduate then
      ...
      if tbOtherStatusInfo then
        bRewardSent = 0 < tbOtherStatusInfo.nRewardCount
      end
    end
    ...
    function pGrid.pPanel.OnTouchEvent()
    ...
    function pGrid.BtnChat.pPanel.OnTouchEvent()
      self:_PrivateChat(tbStudent.nId)
    end
    function pGrid.itemframe.pPanel.OnTouchEvent()
      self:ShowDetailMenu(tbStudent.nId)
    end
  end)
end
function tbUI:_ResetOtherStatusUI()
  ...
  self.pPanel:Label_SetText("TextCondition", string.format("[73cbd5]出师条件：[-]至少完成%d个师徒目标、拜师%d天后", TeacherStudent.Def.nGraduateTargetMin, TeacherStudent.Def.nGraduateConnectDaysMin))
...
function tbUI:_RefreshTimeTip(nOtherId)
  ...
  if nPunishCd > 0 or nAddStudentCd > 0 then
    szTip = nPunishCd > nAddStudentCd and string.format("[73cbd5]解除师徒关系惩罚剩余时间：[-][FFFE0D]%s[-]", Lib:TimeDesc2(nPunishCd)) or string.format("[73cbd5]收徒间隔限制剩余时间：[-][FFFE0D]%s[-]", Lib:TimeDesc2(nAddStudentCd))
  end
  if nOtherId and nOtherId > 0 then
    ...
    if tbOtherStatusInfo and tbOtherStatusInfo.nDismissDeadline and 0 < tbOtherStatusInfo.nDismissDeadline then
...
function tbUI:_SortTargets(tbTargetStates, tbTargets)
  ...
  table.sort(tbRet, function(nIdA, nIdB)
    local nStateA = 0 < (tbTargetStates[nIdA] or 0) and tbTargetStates[nIdA] or TeacherStudent.Def.tbTargetStates.NotFinish
    local nStateB = 0 < (tbTargetStates[nIdB] or 0) and tbTargetStates[nIdB] or TeacherStudent.Def.tbTargetStates.NotFinish
...
function tbUI:RefreshUnderGraduate(tbOtherStatusInfo, tbOtherMainInfo, nConnectDays, bMyTeacher)
  ...
  for nTargetId, nNeedLevel in pairs(TeacherStudent.Def.tbImityTargetsIdToLevels) do
    if tbOtherStatusInfo.tbTargetStates[nTargetId] == 0 then
      tbOtherStatusInfo.tbTargetStates[nTargetId] = nNeedLevel <= nImityLevel and TeacherStudent.Def.tbTargetStates.NotReport or 0
    end
  end
  ...
  self.pPanel:Label_SetText("TextTarget", string.format("[73cbd5]与%s[c8ff00]%s[-]的师徒目标：[-]%d/%d", bMyTeacher and "师父" or "徒弟", tbOtherMainInfo.szName, nFinishedCount, #TeacherStudent.tbTargets))
  ...
  self.pPanel:Label_SetText("TextCondition", string.format("[73cbd5]出师条件：[-]%s至少完成%d个师徒目标、%s拜师%d天后[-]", bValidTarget and szValidColor or szInvalidColor, TeacherStudent.Def.nGraduateTargetMin, bValidDays and szValidColor or szInvalidColor, TeacherStudent.Def.nGraduateConnectDaysMin))
  self.pPanel:SetActive("BtnApprenticeship", bValidDays and bValidTarget)
  if bMyTeacher then
  ...
  self.ConditionScrollView:Update(#tbSortedTargetIds + 1, function(pGrid, nIdx)
    ...
    if nIdx == 1 then
      ...
      function pGrid.Chuangong.pPanel.OnTouchEvent()
        local function fnRequest()
...
function tbUI:RefreshGraduateTeacher(tbOtherMainInfo, tbOtherStatusInfo)
  ...
  local szTips = bAssigned and string.format("[92D2FF]师父[C8FF00]%s[-]布置的任务[-]", tbOtherMainInfo.szName) or string.format("[92D2FF]师父本周还没布置任务[FFFE0D]（挑选%d个）[-]", TeacherStudent.Def.nCustomTaskCount)
  if bReported then
  ...
  self.pPanel:SetActive("BtnCheckA", bAssigned or not not bReported)
  self.pPanel:Button_SetEnabled("BtnReportCustom", bAssigned and not not not bReported)
...
function tbUI:RefreshGraduateStudentTip(bAssigned, tbOtherMainInfo, tbOtherStatusInfo)
  local szMsg = bAssigned and string.format("[92D2FF]已经给徒弟[c8ff00]%s[-]布置任务[-]", tbOtherMainInfo.szName) or string.format("[92D2FF]快给徒弟布置任务吧，已选择：[-]%d/%d", Lib:CountTB(self.tbSelectedTasks or {}), TeacherStudent.Def.nCustomTaskCount)
...
function tbUI:RefreshOtherStatus(nRefreshedId)
  ...
  self.pPanel:Label_SetText("ApprenticeTime", string.format("于%s结为师徒%s", Lib:GetTimeStr3(tbOtherStatusInfo.nConnectTime), nConnectDays > 0 and string.format("（已结为师徒%d天）", nConnectDays) or ""))
