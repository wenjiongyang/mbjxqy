...
function tbUi:RefreshUi(nWeddingLevel)
  ...
  if tbMySchedule and next(tbMySchedule) then
    ...
    if bDue then
      ...
      nWeddingCost = tbMapSetting.nMissCost
    elseif not tbPlayerBookInfo.nOpenTime and nBookLevel == nWeddingLevel then
...
function tbUi:CheckOverdue(nWeddingLevel, tbMySchedule)
  ...
  if nBookLevel == nWeddingLevel and nOpen < nNow and not tbPlayerBookInfo.nOpenTime then
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYNC_WEDDING_SCHEDULE,
      self.SwitchContainer,
      self
    },
    {
      UiNotify.emNOTIFY_WEDDING_DATE_SELECT_FINISH,
      self.OnSelectDateFinish,
      self
    }
  }
  return tbRegEvent
...
tbUi.tbOnClick = {
  ...
  BtnApply = function(self)
    ...
    if tbMySchedule and next(tbMySchedule) then
      ...
      if nOpen < nNow and not tbPlayerBookInfo.nOpenTime then
      ...
      elseif nBookLevel >= self.nLevel then
    ...
    local fnBook = function(self)
      ...
      local szSure = "举行婚礼"
      if self.nLevel == Wedding.Level_2 then
        szTip = string.format("确定将婚期定在[FFFE0D]%s[-]吗？", Lib:TimeDesc11(self.nBookTime))
        szSure = "预定婚礼"
      elseif self.nLevel == Wedding.Level_3 then
        local nWeek = Lib:GetLocalWeek(self.nBookTime)
        local szWeek1 = Lib:TimeDesc11(Lib:GetTimeByWeek(nWeek, 1, 0, 0, 0))
        local szWeek2 = Lib:TimeDesc11(Lib:GetTimeByWeek(nWeek, 7, 0, 0, 0))
        szTip = string.format("确定将婚期定在[FFFE0D]%s ~ %s[-]吗？", szWeek1, szWeek2)
        szSure = "预定婚礼"
      end
