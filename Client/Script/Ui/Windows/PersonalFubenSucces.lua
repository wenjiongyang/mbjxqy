...
function tbUi:Update()
  ...
  local fnSortAward = function(a, b)
  ...
  if nOldStarLevel < 3 and nStarLevel >= 3 then
    ...
    for _, tbInfo in pairs(tbStarAward or {}) do
      if tbInfo.nItemId and tbInfo.nItemId > 0 and tbInfo.nCount and 0 < tbInfo.nCount then
        table.insert(tbAllAward, 1, {
          "item",
          tbInfo.nItemId,
          tbInfo.nCount
        })
...
function tbUi.tbOnClick:BtnNext()
  ...
  if not bRet then
    local tbParam = {
      nSectionIdx = nNextSectionIdx,
      nSubSectionIdx = nNextSubSectionIdx,
...
function tbUi.tbOnClick:BtnFightAgain()
...
function tbUi:RegisterEvent()
  return {
    {
      UiNotify.emNOTIFY_ANIMATION_FINISH,
      self.OnAniEnd,
      self
    }
  }
