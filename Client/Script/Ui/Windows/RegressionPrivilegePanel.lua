...
tbUi.WX_PRIVILEGE_URL = "http://dwz.cn/6jXNeq"
tbUi.QQ_PRIVILEGE_URL = "https://youxi.vip.qq.com/m/act/5ceb06b81e_jxqy_214013.html?_wv=1&_wwv=4"
tbUi.TAB_FREE = 1
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_PRIVILEGE_CALLBACK,
      self.UpdateContent,
      self
    }
  }
...
function tbUi:OnOpen()
  if RegressionPrivilege:IsTriggerByAct(me) then
...
function tbUi:UpdateContent(nTabIdx)
  ...
  local function fnInit(tbItem, nIdx)
...
function tbUi:UpdatePrivilege(tbItem, tbInfo)
  ...
  if tbInfo.szBtnUrl then
    function tbItem.Welcome.BtnReset.pPanel.OnTouchEvent()
...
function tbUi:UpdateFree(tbItem, tbInfo)
  ...
  function tbItem.Return.BtnGet.pPanel.OnTouchEvent()
...
function tbUi:UpdateRecharge(tbItem, tbInfo)
  ...
  function tbItem.Recharge.BtnGet.pPanel.OnTouchEvent()
    if tbInfo.szBtnUrl then
      Ui.HyperTextHandle:Handle(tbInfo.szBtnUrl, 0, 0)
    end
...
function tbUi:UpdateTianJianDesc()
  ...
  tbSubPanel.itemflame2:SetGenericItem({
    "Item",
    RegressionPrivilege.tbTianJian.nItemTID,
    1
  })
  ...
  if bCanGain then
    function tbSubPanel.BtnGuy2.pPanel.OnTouchEvent()
      local nPrice = RegressionPrivilege.tbTianJian.nPrice
      if nPrice > me.GetMoney("Gold") then
  ...
  if nCanBuyID then
    ...
    tbSubPanel.itemflame1:SetGenericItem({
      "Item",
      nCanBuyID,
      1
    })
    tbSubPanel.itemflame1.fnClick = tbSubPanel.itemflame1.DefaultClick
    function tbSubPanel.BtnGuy1.pPanel.OnTouchEvent()
...
function tbUi:GetTimesDesc(nGroup, nParam1, nParam2)
  ...
  if nParam1 == RegressionPrivilege.XIULIAN_TIMES then
    szDesc = string.format("时间：%s/%s小时", nLast / 3600, nMax / 3600)
  end
