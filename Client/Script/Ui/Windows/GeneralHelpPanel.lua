...
local function fnInit()
  ...
  for _, tbInfo in pairs(tbHelpContent) do
    ...
    tbTemp[szKey] = tbTemp[szKey] or {
      szBigTitle = tbInfo.szBigTitle,
      tbContent = {}
    }
    if tbInfo.nHead > 0 then
      tbTemp[szKey].tbGuide = {
        tbInfo.nHead,
        tbInfo.szTitle,
        tbInfo.szContent
      }
    else
      tbTemp[szKey].tbContent[tbInfo.nIndex] = {
        szContent = tbInfo.szContent,
        szTitle = tbInfo.szTitle,
        szFun = tbInfo.Function,
        szFunParam = tbInfo.FunParam
      }
    end
  end
...
local tbColors = {szValid = "00FF00", szInvalid = "FF0000"}
...
tbUi.tbFunction = {
  ...
  AddTeacherHelp = function(self, tbItemContent)
    ...
    local bValidLevel = nLevel <= me.nLevel
    ...
    local szConditions = string.format("[%s]等级达到%s级[-]、[%s]师父数量未满%d人[-]、[%s]当前不处于解除师徒关系惩罚期[-]", getColor(bValidLevel), nLevel, getColor(bValidTeacherCount), TeacherStudent.Def.nMaxTeachers, getColor(bValidNotPunish))
  ...
  AddStudentHelp = function(self, tbItemContent)
    ...
    local szConditions = string.format("[%s]等级达到%s级[-]、[%s]未出师徒弟数量未满%d人[-]、[%s]当前不处于收徒间隔期[-]、[%s]当前不处于解除师徒关系惩罚期[-]", getColor(bValidLevel), nLevel, getColor(bValidStudentCount), TeacherStudent.Def.nMaxUndergraduate, getColor(bValidNoCD), getColor(bValidNotPunish))
    return string.format(tbItemContent.szContent, nMinLvDiff, szConditions)
  end,
