...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_KINGATHER_UPDATE,
      self.Update,
      self
    },
    {
...
function tbUi:Update()
  ...
  local bQuestionBegin = tbQuestionData and 0 < tbQuestionData.nIndex or false
  ...
  nQuotiety = math.min(nQuotiety, Kin.GatherDef.MaxExtraExpBuff)
...
function tbUi:UpdateLastTime()
  local tbData = Kin:GetGatherOtherData() or {}
  local nLastTime = tbData.nLastTime or 600
...
tbUi.tbOnClick = {
  BtnDrink = function(self)
    local tbData = Kin:GetGatherOtherData() or {}
    local nQuotiety = tbData.nQuotiety or 100
    if nQuotiety >= Kin.GatherDef.MaxExtraExpBuff then
      me.CenterMsg("已达到最大喝酒上限！")
      return
    end
    if tbData.bDrink then
      me.CenterMsg("你已经请大家喝过酒了！")
      return
    end
    Kin:GatherDrink()
  end,
