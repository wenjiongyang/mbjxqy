...
function tbUi:GetLimitePValue(nProtentialType, nLimitLevel, nAwareness)
  local nLimitProtential = Partner:GetLimitProtentialValue(self.tbPartnerInfo.nQualityLevel, self.tbPartnerInfo.nGrowthType, nProtentialType, nLimitLevel, self.tbPartnerInfo.nGradeLevel + 1, nAwareness)
...
function tbUi:SetProtential(szTitle, tbProtentialInfo, tbOther)
  for i, szType in ipairs(self.tbProtentialList) do
    ...
    local fnGetValue = function(nOldV, nV)
...
function tbUi.tbOnClick:BtnBefore()
...
function tbUi.tbOnClick:BtnAfter()
  Ui:CloseWindow(self.UI_NAME)
