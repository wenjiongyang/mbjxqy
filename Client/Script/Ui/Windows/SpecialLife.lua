...
function tbUi:GetPosInfo(nPercent)
  ...
  if nCount > 0 and math.abs(nShowPt) <= 1.0E-5 then
    nCount = nCount - 1
    nShowPt = 1
  end
...
function tbUi:SetPercent(nPercent, bNotClose)
  ...
  if not bNotClose and self.nCurPercent <= 5.0E-5 then
    Ui:CloseWindow("SpecialLife")
  end
end
function tbUi:UpdateBlood()
  Log("UpdateBlood -->> ", self.nCurPercent or 0, self.nRealPercent or 0)
  if math.abs(self.nCurPercent - self.nRealPercent) <= 5.0E-5 then
    self.nTimerId = nil
    self:SetPercent(self.nRealPercent)
    return
  end
  ...
  self:SetPercent(self.nCurPercent - 1.0E-5, true)
