...
function tbUi:Reset()
  ...
  for i, pItem in ipairs(tbAllZhenYuans) do
    if 0 < pItem.GetSingleValue() then
...
function tbUi:UpdateRigtItemList()
  local function fnClick(itemObj)
    if itemObj.bCanAdd then
      ...
      local tbSkillInfo = Item:GetClass("ZhenYuan"):GetSkillAttribTip(pCurEquip)
      local nSkillID, nSkillLevel = unpack(tbSkillInfo)
      if nCurMaxLevel <= nSkillLevel then
        me.CenterMsg("当前已满级")
        return
      end
  ...
  self.ScrollView2:Update(math.max(math.ceil(#self.tbRightItemList / ITEM_PER_LINE), MIN_LINE_Right), fnSetItem)
end
...
function tbUi:UpdateLeftItemList()
  ...
  self.ScrollView1:Update(math.max(math.ceil(#self.tbLeftItemList / ITEM_PER_LINE), MIN_LINE_Left), fnSetItem)
  local pCurEquip = me.GetEquipByPos(Item.EQUIPPOS_ZHEN_YUAN)
