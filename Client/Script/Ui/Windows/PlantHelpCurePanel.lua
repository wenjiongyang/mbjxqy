...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYNC_FRIEND_PLANT,
      self.OnSyncData,
      self
    }
  }
...
function tbGrid.tbOnClick:BtnVisit()
...
function tbGrid:SetData(tbData)
  ...
  if tbFriend then
    ...
    local szPortrait, szAltas = PlayerPortrait:GetSmallIcon(tbFriend.nPortrait or tbFriend.nFaction)
    self.pPanel:Sprite_SetSprite("SpRoleHead", szPortrait, szAltas)
    local tbHonorInfo = Player.tbHonorLevelSetting[tbFriend.nHonorLevel]
