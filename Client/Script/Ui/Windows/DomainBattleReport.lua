...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_ONSYNC_DOMAIN_REPORT,
      self.RefreshUi,
      self
    }
  }
...
tbUi.tbOnClick = {
  ...
  BtnChange = function(self)
    ...
    for i, v in ipairs(tbInfo.tbAtackPos) do
      table.insert(OptList, {
        Text = string.format("更换为：%s", v[4] and v[4] or "营地" .. i),
        Callback = fnSelectCamp,
        Param = {i}
      })
    end
    Dialog:Show({
      Text = "被围堵了吗？没关系，更改征战营地吧！家族成员下次重生时将出现在更改后的征战营地。",
      OptList = OptList
    }, me, {
      szName = "家族总管",
      nTemplateId = 627
