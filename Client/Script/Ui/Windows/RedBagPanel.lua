...
function tbUI:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_REDBAG_DATA_REFRESH,
      self.Refresh,
      self
    }
  }
...
function tbUI:GetRedBags(bMine)
  ...
  for _, tb in ipairs(Kin.tbRedBags or {}) do
    ...
    elseif tb.tbOwner.nId ~= me.dwID then
      table.insert(tbRet, tb)
    end
  end
  return tbRet
end
function tbUI:ShowItemBg(pGrid, szNotEmpty, szEmpty, bEmpty, bGlobal)
  ...
  self.RecordScrollView:Update(nRows, function(pGrid, nIdx)
    for i = 1, 3 do
      ...
      if tbRedBag then
        ...
        self:ShowItemBg(pGrid, szNotEmpty, szEmpty, nState == tbRedBagStates.empty, bGlobal)
...
function tbUI:RefreshOthers()
  ...
  self.RedPaperScrollView:Update(nCount, function(pGrid, nIdx)
    ...
    self:ShowItemBg(pGrid, "RedPaper1", "RedPaper2", tbRedBag.bEmpty, bGlobal)
    if tbRedBag.bCanGrab then
      pGrid.BtnGrant.pPanel:SetActive("Main", true)
      function pGrid.BtnGrant.pPanel.OnTouchEvent()
      ...
      function pGrid.BtnCheck.pPanel.OnTouchEvent()
