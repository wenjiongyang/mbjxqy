...
function tbUi:OnOpen(nItemTemplateId, nItemId, tb4ChooseList)
  ...
  for _, tbItemChoose in pairs(tb4Choose) do
    if 0 < tbItemChoose.nThisNeedCount and nNeedCount > tbItemChoose.nThisNeedCount then
      nNeedCount = tbItemChoose.nThisNeedCount
    end
  end
  if nNeedCount > 0 and nNeedCount > me.GetItemCountInAllPos(nItemTemplateId) then
...
function tbUi:UpdateList()
  ...
  local function fnSet(item, nIdx)
    ...
    for i = 1, 2 do
      ...
      local szName = KItem.GetItemShowInfo(nItemTID, me.nFaction)
      ...
      if bMultiChoose then
        function pItem.BtnMinus.pPanel.OnTouchEvent()
          self:ChangeNum(pItem, nItemTID, -1)
        end
        function pItem.BtnPlus.pPanel.OnTouchEvent()
          self:ChangeNum(pItem, nItemTID, 1)
        end
        pItem.Label_Number.pPanel:Label_SetText("Main", "0")
        function pItem.Label_Number.pPanel.OnTouchEvent()
        ...
        function pItem.pPanel.OnTouchEvent()
...
function tbUi:OnSelectItem(item, nItemTemplateId)
  ...
  elseif self.nCanChooseNum == 1 then
