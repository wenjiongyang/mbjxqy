...
function tbUi:Update()
  ...
  for i = 1, self.nOtherNpcCount or 0 do
    local tbInfo = {}
    tbInfo.szName = Login:GetRandomName()
  ...
  local function fnSetItem(ItemObj, index)
    ...
    ItemObj.pPanel:Label_SetText("Relationship", 0 < (tbInfo.nImity or 0) and "好友" or "陌生人")
    ...
    local tbHonor = self.tbHonorInfo[tbInfo.nHonorLevel or 0]
    if not tbHonor then
      ...
      ItemObj.pPanel:Sprite_Animation("PlayerTitle", tbHonor.ImgPrefix)
    end
    local szFactionIcon = Faction:GetIcon(tbInfo.nFaction)
    ItemObj.pPanel:Sprite_SetSprite("SpFaction", szFactionIcon)
    local nAward = 0 < (tbInfo.nImity or 0) and Helper.FRIEND_JADE or Helper.STRANGER_JADE
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_HELPER_GET_STRANGER,
      self.OnGetStrangerList
    },
    {
      UiNotify.emNOTIFY_HELPER_GET_SYNCDATA,
      self.OnGetSyncData
    }
  }
  return tbRegEvent
...
function tbUi.tbOnClick:BtnSkip()
