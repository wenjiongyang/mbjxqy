...
function tbGrid:SetData(tbRole, tbParent)
  ...
  local szPortrait, szAltas = PlayerPortrait:GetSmallIcon(tbRole.nPortrait or tbRole.nFaction)
...
local GetProbLevel = function(nProb)
  if nProb < 0.15 then
    return 1
  ...
  for i, tbRole in ipairs(tbData) do
    ...
    if tbRole.dwID == 0 then
      ...
      if not tbRole.szName then
        tbRole.szName = Login:GetRandomName()
      end
...
function tbUi:RegisterEvent()
  return {
    {
      UiNotify.emNOTIFY_SYNC_DEBRIS_ROB_DATA,
      self.OnOpen
    },
    {
