...
tbUi.tbOpenUiFunc = {
  ...
  DomainCityInfo = {
    nInterval = 3600,
    fnIsExc = function(self)
      return self.nMapTemplateId == DomainBattle.define.tbMasterStatuePos[1] and GetTimeFrameState(DomainBattle.define.szOpenTimeFrame) == 1
    end,
    fnExc = function(self, bInterval)
      if not bInterval then
        return
...
function tbUi:OnOpen(nMapTemplateId)
  ...
  self.tbMainMapSetting = tbMiniMapSetting
  ...
  self.bWtfMap = Fuben.WhiteTigerFuben:IsMyMap()
  ...
  for k, v in pairs(self.tbOpenUiFunc) do
    if v.fnIsExc(self) then
      ...
      if nNow > (self.nRequestTime or 0) + v.nInterval then
        bInterval = true
  ...
  if self.bInFightMap or self.bWtfMap or self.bIsBattle then
    ...
    for i = 1, self.nMaxAttackMeCount do
      if tbAttackMePos[i] then
        self:ShowPos(szNodeName .. i, tbAttackMePos[i].nX, tbAttackMePos[i].nY)
      else
...
function tbUi:GetAllHouse(pPlayer)
  ...
  if House.bHasHouse then
    table.insert(tbHouse, {
      dwOwnerId = pPlayer.dwID,
      szName = "我的家园"
    })
  end
...
tbUi.tbOnClick = {}
function tbUi.tbOnClick:BtnHome()
