...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYNC_EVERYDAY_TARGET,
      self.UpdateDailyTargetState,
      self
    },
    {
      UiNotify.emNOTIFY_ONACTIVITY_STATE_CHANGE,
...
function tbUi:UpdatePreviewInfoTxt()
  ...
  if tbPreViewInfo.nWillOpenDay > 0 then
    ...
    self.pPanel:Label_SetText("Txt3", string.format("[92d2ff]距离开放%d级上限：[-]%d天后（%d月%d日 4:00）", tbPreViewInfo.nNextMaxLevel, tbPreViewInfo.nWillOpenDay, tbTime.month, tbTime.day))
  else
...
function tbUi:UpdateDailyTargetState()
  ...
  for nIdx, nValue in ipairs(EverydayTarget.Def.tbActiveScale) do
    ...
    local bComplete = nValue <= nTotalValue
...
function tbUi:UpdateTargetAward(nIdx)
  local tbAward = {
    unpack(EverydayTarget.tbShowAward[nIdx] or {})
  }
...
function tbUi:UpdateWeekItem(itemObj)
  ...
  for _, tbInfo in ipairs(tbOpenTime) do
    ...
    if nCurTime >= nOpenTime - 3600 and nCurTime < nOpenTime then
...
function tbUi:UpdateDaily(bDir2Act)
    ...
    for i = 1, nItemNum do
      ...
      if tbAct then
        ...
        do
          ...
          function child.pPanel.OnTouchEvent()
  ...
  local function fnSetTimeItem(itemObj, nIdx)
...
local UpdateActDetail = function(self, nActivityID)
  local szTime, szType, szDesc, szHelpKey = Calendar:GetActivityDetail(nActivityID)
  if not (szTime and szType and szDesc) or not szHelpKey then
...
function ActivityTip.tbOnClick:BtnBg()
  Ui:CloseWindow(self.UI_NAME)
end
local fnRegisterRedpoint = function(pPanel, szNodeName, szKey)
...
function tbTime:UpdateSideTip(tbData, bPreview)
  ...
  if not bPreview and fnSideTips then
    ...
    function self.BtnGo.pPanel.OnTouchEvent()
