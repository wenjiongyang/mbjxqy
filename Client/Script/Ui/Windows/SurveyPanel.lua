...
function tbUI:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SURVEY_SEL_INPUT_CHANGE,
      self.OnSurveySelInputChange,
      self
    }
  }
  return tbRegEvent
  ...
  local function fnSetItem(pGrid, nIdx)
    for i = 1, 2 do
      ...
      if bValid then
        ...
        function pBtn.TypeIcons.pPanel.OnTouchEvent(pSender)
...
function tbUI:ShowMatrix()
  ...
  self.MatrixGroup:Update(nRows, function(pGrid, nIdx)
    ...
    for i = 1, 11 do
      ...
      function pSelect.pPanel.OnTouchEvent()
        for j = 1, 11 do
...
function tbUI:SubmitSingleSel()
  ...
  if nCurSel <= 0 then
    ...
    local bInput = tbChoice.nOther and 0 < tbChoice.nOther
    local szInput
    if bInput then
