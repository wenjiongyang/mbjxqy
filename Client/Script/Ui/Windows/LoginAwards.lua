...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_LOGINAWARDS_CALLBACK,
      self.Update,
      self
    }
  }
...
function tbUi:UpdateCountDown()
  ...
  local nLoginDay = me.GetUserValue(LoginAwards.LOGIN_AWARDS_GROUP, LoginAwards.LOGIN_DAYS)
  local szTimeTitle = nLoginDay >= 3 and "三阶稀有首饰礼盒领取倒计时" or "月眉儿领取倒计时"
...
function tbItem:Update(bAct, nIdx, fnOnClick)
  ...
  self.pPanel:SetActive("BtnGet", nDayFlag == 0 and nIdx <= nLoginDay)
  self.pPanel:SetActive("BtnLabel", nDayFlag == 0 and nIdx <= nLoginDay)
  ...
  if bAct then
    local tbTime = os.date("*t", LoginAwards.nActBeginTime + 86400 * (nIdx - 1))
