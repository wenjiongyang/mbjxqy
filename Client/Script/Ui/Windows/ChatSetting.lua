...
tbUi.tbTabData = {
  [tbUi.Ui_Type.MsgSetting] = {
    fnUpdate = function(self)
      self:UpdateMsgSettingUi()
    end,
    szPanel = "ChatPart",
    szToggle = "BtnChatSetting",
    szTitle = "聊天设置"
  },
  [tbUi.Ui_Type.ChatDecorate] = {
    ...
    szPanel = "HeadFramePart",
    szToggle = "BtnPersonalitySetting",
    szTitle = "个性设置"
  }
}
  ...
  local function fnSetItem(itemObj, nIdx)
    for i = 1, nRowNum do
      ...
      if nThemeInfo then
        ...
        itemObj.pPanel:Label_SetText("Name" .. i, ChatDecorate:GetTitleByThemeId(nThemeID, me.nFaction))
        local nEndTime = ValueItem.ValueDecorate:GetValue(me, nThemeID)
        local nNowTime = GetTime()
        local bTime = nEndTime > 0 and nEndTime > nNowTime
        ...
        for nPartsType, nPartsID in ipairs(nThemeInfo.tbParts or {}) do
          ...
          local szGet = ChatDecorate:GetTips(nPartsID, me.nFaction)
          if nPartsType == ChatDecorate.PartsType.HEAD_FRAME then
            local nIcon = ChatDecorate:GetIcon(me.nFaction, nPartsID)
            local szFrameAtlas, szFrameSprite = Item:GetIcon(nIcon)
            local szRoleHead, szRoleAtlas = PlayerPortrait:GetSmallIcon(me.nPortrait or me.nFaction)
            if bCanApply then
          ...
          function obj.pPanel.OnTouchEvent(childObj)
            if not bCanApply then
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_CHAT_THEME_OVERDUE,
      self.UpdateChatDecorateUi
    }
  }
  return tbRegEvent
