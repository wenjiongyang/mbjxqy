...
local tbItemUi = Ui:CreateClass("DoubleItem")
tbUi.szCmd = ""
tbItemUi.nBtnNum = 2
...
function tbUi:AddRecentCmd(szCmd)
  if string.find(szCmd, "-- ReloadLuaFile --") then
    return
  end
...
function tbUi:RefreshRecentList()
  local function fnOnClick(index)
...
function tbUi.tbOnClick:BtnSave()
  ...
  elseif nChange > 10 then
    for i = nChange, 2, -1 do
      self.tbTextList[i] = {
        self.tbTextList[i - 1][1],
        self.tbTextList[i - 1][2]
      }
    end
...
function tbUi.tbOnClick:BtnStatistic()
