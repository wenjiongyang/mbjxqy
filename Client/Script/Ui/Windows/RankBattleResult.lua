...
function tbRankResult:OnOpen(nResult, varMe, varEnemy, tbDamageCounter, tbCallback, szBattleClassType)
  ...
  local bHideFail = szBattleClassType == "BossFightBattle" or szBattleClassType == "BossFight_Client"
  if nResult == 1 then
...
function tbRankResult:SetPlayerShow(pAsyncData, nCamp)
  ...
  for i = 1, 4 do
    local nIdx = i + 1
    local nPartnerId, nLevel, nFightPower = pAsyncData.GetPartnerInfo(i)
