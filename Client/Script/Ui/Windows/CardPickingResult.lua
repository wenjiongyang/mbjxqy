...
function tbUi:OnOpenEnd(szPickingType)
  for i = 0, 10 do
    self["Item" .. i].pPanel:PlayUiAnimation("TurnoverCardReset", false, false, {
      tostring(self["Item" .. i].pPanel)
    })
  end
  tbUi.bClose = false
...
function tbUi:Show1Result(tbResults)
  ...
  self.Item0.pPanel:PlayUiAnimation("ChouKaChuangKouDongHua", false, false, {
    tostring(self.Item0.pPanel)
  })
...
function tbUi:Show10Result(tbResults)
  ...
  for nIth, tbItem in ipairs(tbResults) do
    Timer:Register(nIth * 5 - 4, function()
      ...
      self["Item" .. nIth].pPanel:PlayUiAnimation("ChouKaChuangKouDongHua", false, false, {
        tostring(self["Item" .. nIth].pPanel)
      })
      local bRet = pcall(self["Item" .. nIth].Init, self["Item" .. nIth], tbItem.szItemType, tbItem.nItemId, fnCallback, Partner.tbDes2QualityLevel.A, tbItem.nPartnerId)
      if not bRet then
