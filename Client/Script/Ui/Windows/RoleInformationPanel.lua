...
RoleInformationPanel.tbOnClick = {
  ...
  BtnBinding = function(self)
    if Sdk:IsMsdk() then
      local szUrlFormat = "http://jxqy.qq.com/ingame/all/act/a20170317jxyzm/index.html?roleid=%d&serverid=%d&platid=%d&area=%d&loginplatid=%d"
    ...
    elseif version_xm then
      local szUrlFormat = "https://m.efunen.com/bindPhone.html?uid=%s&sign=%s&timestamp=%s&language=zh_CH&gameCode=%s"
  ...
  CheckSoundEffect = function(self)
    ...
    local fVolume = tbUserSet.fSoundEffectVolume > 0.001 and 0 or 1
    Ui:SetSoundEffect(fVolume, true)
  end,
  CheckMusic = function(self)
    local tbUserSet = Ui:GetPlayerSetting()
    local fVolume = tbUserSet.fMusicVolume > 0.001 and 0 or 1
  ...
  Btn01 = function(self)
    Sdk:OpenUrl("http://www.tencent.com/en-us/zc/privacypolicy.shtml")
  end,
  Btn02 = function(self)
    Sdk:OpenUrl("http://www.tencent.com/en-us/zc/termsofservice.shtml")
  end,
  Btn03 = function(self)
    Sdk:OpenUrl("http://game.qq.com/contract.shtml")
  end,
  Btn04 = function(self)
    local nNow = GetTime()
    if self.nLastSendLogTime and nNow < self.nLastSendLogTime + 60 then
      me.CenterMsg(XT("上报失败请稍后再试。"))
      return
    end
    self.nLastSendLogTime = nNow
    FileServer:SendClientLog()
  end,
  ...
  BtnWebsite = function(self)
    Sdk:OpenUrl("http://sword.efuntw.com/play/")
  end,
  BtnHabamute = function(self)
    Sdk:OpenUrl("http://forum.gamer.com.tw/A.php?bsn=28903")
  end,
  BtnCredit = function(self)
    ...
    local szUrlFormat = "https://wx.gamesafe.qq.com/credit_game_plat?game_id=2082&area=%d&platid=%d&partition=%d&charac_no=%d&role=%s"
    local nArea = Sdk:GetAreaId()
    local nPlatId = Sdk:GetPlatId()
    local nServerId = Sdk:GetServerId()
    local szUrl = string.format(szUrlFormat, nArea, nPlatId, nServerId, me.dwID, me.szName)
    Sdk:OpenUrl(szUrl)
  end
}
...
function RoleInformationPanel:OnOpen(bIsShowBindingGuide)
  ...
  if Sdk:IsMsdk() then
    ...
    self.pPanel:SetActive("BtnShowOff", Sdk:IsEfunHKTW() or version_xm)
    self.pPanel:SetActive("BtnCustomerService", version_vn and true or false)
    self.pPanel:SetActive("BtnCustomerService1", version_vn and true or false)
    ...
    self.pPanel:SetActive("Btn02", false)
    self.pPanel:SetActive("Btn03", false)
...
function RoleInformationPanel:UpdateNotifySetting()
  ...
  local function fnClickToggle(tbToggle)
...
function RoleInformationPanel:UpdateDrawLevelValue(bUpdateLevel)
  ...
  self.pPanel:Label_SetColorByName("LbLowDraw", "White")
  self.pPanel:Label_SetColorByName("LbMiddleDraw", "White")
  self.pPanel:Label_SetColorByName("LbHeightDraw", "White")
  if fValue >= RoleInformationPanel.fDrawDivide * 2 then
    nDrawLevel = Ui.tbDefDrawLevel.nHeight
    self.pPanel:Label_SetColorByName("LbHeightDraw", "Green")
  elseif fValue >= RoleInformationPanel.fDrawDivide then
    nDrawLevel = Ui.tbDefDrawLevel.nMiddle
    self.pPanel:Label_SetColorByName("LbMiddleDraw", "Green")
  else
    nDrawLevel = Ui.tbDefDrawLevel.nLow
    self.pPanel:Label_SetColorByName("LbLowDraw", "Green")
  end
...
function RoleInformationPanel:UpdateDrawSetting()
  ...
  if tbUserSet.nDrawLevel == Ui.tbDefDrawLevel.nHeight then
    self.pPanel:SliderBar_SetValue("BtnBarDraw", 1)
  ...
  elseif tbUserSet.nDrawLevel == Ui.tbDefDrawLevel.nLow then
    self.pPanel:SliderBar_SetValue("BtnBarDraw", 0)
  end
  self:UpdateDrawLevelValue()
...
function RoleInformationPanel:UpdateRenderPlayerCount(bSetValue)
  ...
  fValue = math.min(1, fValue)
  fValue = math.max(0, fValue)
  if fValue >= 0.98 then
...
function RoleInformationPanel:UpdateSoundSetting()
  ...
  if 0.001 >= tbUserSet.fMusicVolume then
...
function RoleInformationPanel:UpdatePortrait()
  ...
  self.pPanel:Sprite_SetSprite("Head", tbSetting.szBigIcon or "", tbSetting.szBigIconAtlas)
end
function RoleInformationPanel:UpdateHonorLevelTitle()
  ...
  if tbHonorInfo then
    ...
    self.pPanel:Sprite_Animation("lbRoleTitle", tbHonorInfo.ImgPrefix)
  else
...
function RoleInformationPanel:RegisterEvent()
  local tbRegEvent = {
    ...
    {
      UiNotify.emNOTIFY_CHANGE_PLAYER_NAME,
      self.UpdateName
    },
    {
    ...
    {
      UiNotify.emNOTIFY_ONLINE_ONHOOK_STATE,
      self.UpdateOnlineOHSetting
    },
    {
