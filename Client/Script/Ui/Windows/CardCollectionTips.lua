...
function tbUi:OnOpenEnd(nItemTemplateId, nItemId, tbIntValueInfo)
  ...
  if nItemId then
    ...
    for i = 1, nLen do
      ...
      local bActivate = 0 < CollectionSystem:GetDecimalBits(nFlag, nSavePos)
  ...
  self.szCollectionInfo = nCollectionId == CollectionSystem.RANDOMFUBEN_ID and string.format("珍稀度：%d\n完成度：%d/%d", nRare, nCompletion, nShowLen) or string.format("完成度：%d/%d", nCompletion, nShowLen)
  if bRankBoard then
    if nItemId then
      self:UpdateRankInfo()
      RankBoard:CheckUpdateData("CardCollection_" .. nCollectionId, 1)
    else
      local szPos = 0 < (tbIntValueInfo[CollectionSystem.ITEM_RANK] or 0) and tbIntValueInfo[CollectionSystem.ITEM_RANK] or "未上榜"
