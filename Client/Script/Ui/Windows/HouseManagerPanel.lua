...
tbUi.DESC_COMFORT_PRIVILEGE = "●  家园冥想时基础元气获得[FFFE0D]增加%d%%[-]\n●  传功时，传功者和被传功者经验获得[FFFE0D]增加%d%%[-]\n"
tbUi.DESC_ROOMER_PRIVILEGE = "●  房客的家园享有[FFFE0D]%d级[-]舒适等级加成\n●  享有在本家园内的拍照功能\n\n"
tbUi.DESC_ROOMER_INVALID = "[FF6464FF]注：房客特权凌晨4:00以后生效[-]\n"
tbUi.DESC_EXT_COMFORTLEVEL = "[FF6464FF]注：当前你的家园实际享有%d级舒适等级加成[-][00FF00][url=openwnd:了解房客特权, GeneralHelpPanel, \"RoomeHelp\"][-]\n"
  ...
  if House.nStartLeveupTime and House.nStartLeveupTime > 0 then
    ...
    if nLeftTime > 0 then
      ...
      self.nUpdateLeveupTimerId = Timer:Register(Env.GAME_FPS, function()
        ...
        if nLastTime <= 0 then
          self.LevelUpTime:SetLinkText("[C8FF00]家园扩建升级已完成，请前往[url=npc:真妹, 2279, 10]处确认[-]")
...
function tbUi:UpdateComfortList()
  ...
  local function fnSetItem(tbItem, i)
    local nTotalCount = tbComfort[i] or 0
    local szTypeName = tbFurniture[i].szName
    tbItem.pPanel:Label_SetText("Name", string.format("[92D2FF]%s[-]", szTypeName))
    local nCurCount = math.min(#(self.tbMapFurniture[i] or {}), nTotalCount)
    tbItem.pPanel:Label_SetText("Number", string.format("[%s]（%s/%s）[-]", nCurCount == nTotalCount and "00FF00" or "FFFFFF", nCurCount, nTotalCount))
    function tbItem.pPanel.OnTouchEvent(tbObj)
      if not self.tbMapFurniture[i] then
        Ui:AddCenterMsg("家园内未摆放此类型家具")
        return
      end
      self:ResetComfortSelect()
      self:SelectComfortType(i)
      Ui:OpenWindowAtPos("HouseComfortableDetailsPanle", 28, -2, szTypeName, self.tbMapFurniture[i], function()
        self:ResetComfortSelect()
      end)
    end
  end
...
function tbUi:UpdateRoomPanel(bIsInOwnHouse)
  ...
  for i = 0, self.MAX_ROOMER_COUNT do
    ...
    if bHasRoomer then
      local tbSetting = PlayerPortrait:GetPortraitSetting(tbRoomer.nPortrait)
      self.pPanel:Sprite_SetSprite(szRoleHead, tbSetting.szBigIcon or "", tbSetting.szBigIconAtlas)
...
function tbUi:BtnToggle(nType)
  if nType ~= House.nAccessType_Friend and nType ~= House.nAccessType_Kin and nType ~= House.nAccessType_Stranger then
    return
  end
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYNC_HOUSE_ACCESS,
      self.OnSyncAccess
    },
    {
    ...
    {
      UiNotify.emNOTIFY_ZHEN_YUAN_MAKE,
      self.OnMakeResult
    },
    {
      UiNotify.emNOTIFY_SYNC_EXT_COMFORTLEVEL,
      self.OnSyncExtComfortLevel
    }
  }
  return tbRegEvent
end
...
function tbUi:SelectRoomer(nIndex)
  ...
  Ui:OpenWindowAtPos("RightPopup", tbPos.x + 190, tbPos.y - 320, "HouseRoomerSelect", {
    dwRoleId = tbRoomer.dwPlayerId,
    szRoleName = tbRoomer.szName
  }, function()
    self:ResetRoomerSelect()
  end)
end
...
function tbUi.tbOnClick:BtnLevelUp()
  ...
  if nMaxLevel <= House.nHouseLevel then
    return
  end
  local tbHouseSetting = House.tbHouseSetting[House.nHouseLevel]
  Ui:OpenWindow("MessageBox", string.format("本次家园升级需花费[FFFE0D]%d元宝[-]\n确定升级吗？", tbHouseSetting.nLevelupCost), {
    {
      function()
...
function tbUi.tbOnClick:BtnManage()
  self:ChangePanel("Manage")
end
function tbUi.tbOnClick:BtnWing()
  self:ChangePanel("Wing")
end
function tbUi.tbOnClick:BtnComfort()
  self:ChangePanel("Comfort")
end
function tbUi.tbOnClick:BtnVitality()
  self:ChangePanel("Vitality")
end
function tbUi.tbOnClick:BtnMeditation()
  RemoteServer.TryMuse()
  Ui:CloseWindow(self.UI_NAME)
end
function tbUi.tbOnClick:BtnLeave()
  Ui:OpenWindow("MessageBox", "确定要搬离吗？", {
    {
      function()
