...
function tbUi:CheckPartnerUiOK()
  ...
  if not (tbPMain.nCurPartnerId and self.nPartnerId) or self.nPartnerId ~= tbPMain.nCurPartnerId then
    return false
  end
...
function tbUi:Update()
  ...
  for i = 1, 3 do
    ...
    local szName, nIcon = KItem.GetItemShowInfo(nItemTemplateId)
...
function tbUi:DoPlayAni()
  ...
  if not (bRet and self.bIsPress) or nCostCount >= self.nMaxCount or nCurItemCount <= 0 or self.nLevel >= Partner.MAX_LEVEL then
