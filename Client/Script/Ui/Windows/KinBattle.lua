...
function tbUi:Update()
  for i = 1, self.MAX_COUNT do
    ...
    for nType = 1, KinBattle.nKinBattleTypeCount do
      ...
      self.pPanel:SetActive("WaitingOpen" .. nIndex, not tbCurResult or not tbCurResult.szOtherKinName)
    ...
    self.pPanel:SetActive("Time" .. i, not tbCurResult or not tbCurResult.bIsFinish)
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYNC_KIN_BATTLE_DATA,
      self.OnSyncKinBattleData,
      self
    }
  }
  return tbRegEvent
...
function tbUi.tbOnClick:BtnSet()
  self:SetLevelLimite()
end
