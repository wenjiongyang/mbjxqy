...
function tbUI:SetCount(nToCount, bAjustment)
  ...
  if bAjustment then
    ...
    nToCount = nMax
    nToCount = nRemainCount and nRemainCount < nToCount and nRemainCount or nToCount
    nToCount = nToCount > 0 and nToCount or 1
  end
  local nHasMoney = me.GetMoney(szMoneyType)
  if nHasMoney < nPrice * nToCount then
    me.CenterMsg("购买数量已达上限")
    return false
  end
  if nRemainCount and nRemainCount < nToCount then
    me.CenterMsg("库存不足")
    return false
  end
...
function tbUI:UpdateRightPanel()
  ...
  if self.tbSelectItem then
    ...
    if nHasMoney >= nPrice * nCount then
      self.pPanel:Label_SetColorByName("TxtCostMoney", "White")
    else
...
function tbUI:Refresh()
  ...
  self.RenownGoods:Update(nRows, function(pGrid, nIdx)
    for i = 1, 2 do
      ...
      if tbItem then
        local szName = KItem.GetItemShowInfo(tbItem.nItemId, me.nFaction)
        ...
        pItem.Item:SetItemByTemplate(tbItem.nItemId, nil, me.nFaction, {bShowCDLayer = bSoldOut})
