...
function tbUi:OnOpenEnd(tbResult)
  ...
  local szIcon, szIconAtlas = PlayerPortrait:GetPortraitIcon(tbTarget.nPortrait or tbTarget.nFaction)
  self.RoleContainer:InitHead(szIcon, szIconAtlas, tbTarget.nLevel, tbTarget.nFaction, tbResult.bMainBeated or false)
  local tbHonorInfo = Player.tbHonorLevelSetting[tbTarget.nHonorLevel]
