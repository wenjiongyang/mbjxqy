local tbUi = Ui:CreateClass("MessageBoxBig")
tbUi.tbOnClick = {
...
function tbUi:OnOpen(szText, tbProc, tbBtnTxt, nPivot, szNotTipsType, nTime, bInfoLayer, tbLight)
  local szInfo = string.format(szText, nTime or 0)
  if nPivot then
    self.pPanel:ChangePivot("TextInfo", nPivot)
  elseif string.find(szInfo, "#%d+") then
    self.pPanel:ChangePivot("TextInfo", 0)
  else
    self.pPanel:ChangePivot("TextInfo", 4)
  end
  self.pPanel:Label_SetText("TextInfo", szInfo)
  if nTime and nTime > 0 then
    ...
    self.nTimerId = Timer:Register(Env.GAME_FPS, function()
      ...
      self.pPanel:Label_SetText("TextInfo", string.format(szText, nCurTime))
      return true
    end)
  end
  if tbProc then
  ...
  local isSingle = tbProc and #tbProc == 1 or false
