...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_MS_ITEM_LIST_CHANGE,
      self.OnRefreshItemList,
      self
    },
    {
      UiNotify.emNOTIFY_MS_MY_ITEM_LIST_CHANGE,
    ...
    {
      UiNotify.emNOTIFY_MS_ITEM_SOLD,
      self.RefreshGroup,
      self
    },
    {
      UiNotify.emNOTIFY_MARKET_STALL_REFRESH_ALL,
      self.RefreshAllMarket,
      self
    },
    {
      UiNotify.emNOTIFY_MS_HAS_LOWER_PRICE,
      self.OnHasLowerPrice,
      self
    }
  }
  return tbRegEvent
  ...
  me.MsgBox("[FFFE0D]小提示：当前摊位中有更低价的商品哦。[-]", {
    {
      "前往查看",
      function()
        Ui:OpenWindow("MarketStallBuyPanel", {
...
function tbUi:InitUi()
  ...
  function self.Toggle.pPanel.OnTouchEvent()
...
local tbSubMenuStatus = {Hide = 1, Show = 2}
function tbUi:_UpdateMenuData()
  self.nCurTypeId = self.nCurTypeId or 1
  local tbMenuItems = {}
...
function tbUi:UpdateItemTypeList()
  ...
  local function fnSetItem(pGrid, nIdx)
    ...
    if bSubmenuItem then
      ...
      function pGrid.SubClass.pPanel.OnTouchEvent()
        self:SelectMarketType(tbData.nTypeId, tbData.tbSubTypes)
      end
    else
      local szIcon = tbData.nTypeId == self.nCurTypeId and "BtnListMainPress" or "BtnListMainNormal"
      ...
      function pGrid.BaseClass.pPanel.OnTouchEvent()
...
function tbUi:OnOpenEnd(nType, nCurTypeId, szMainType, nSelectItemSubType)
  local bMarketOpen, err = self:CheckMarketOpen()
  if nType and not bMarketOpen then
    me.CenterMsg(err)
    return
  end
...
function tbUi.fnClickGrid(tbGrid)
  local tbBaseInfo = KItem.GetItemBaseProp(tbGrid.nTemplate)
  if tbBaseInfo.szClass == "Stone" then
    Item:ShowStoneCompareTips(tbGrid.nTemplate, tbGrid.nItemId)
  else
  ...
  local function fnSetItem(tbItemGrid, index)
    ...
    for i = 1, 2 do
      ...
      if tbData then
        ...
        function it.pPanel.OnTouchEvent()
...
function tbUi:RefreshGroup(bNeedRefresh)
  ...
  for i = 1, 8 do
    ...
    if tbData then
      ...
      function itemGrid.pPanel.OnTouchEvent()
    ...
    for i = 1, 2 do
      ...
      if tbItem then
        ...
        if tbItem.szIcon and tbItem.szAtlas then
          ...
          local tb = MarketStall:GetStallAward(tbItem.szMainType, tbItem.nSubType, 0)
          pItem.Item:SetGenericItem(tb)
        end
        local function _fnEnterGrp()
          fnEnterGrp(tbItem)
        end
        pItem.Item.fnClick = self.fnClickGrid
        pItem.pPanel.OnTouchEvent = _fnEnterGrp
      end
    end
  end)
...
function tbUi:SelectMarketType(nTypeId, tbSubType)
  ...
  elseif nTypeId == self.nExpandedSubmenu then
    self.nExpandedSubmenu = nil
  else
