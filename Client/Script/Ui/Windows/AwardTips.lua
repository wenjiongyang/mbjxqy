...
local ANI_MOVE_TIME = 2
local FULL_WIDTH = 550
local FULL_HEIGH = 330
function tbUi:OnOpen(tbAward, nLogWay)
  ...
  for i, v in ipairs(tbAward) do
    ...
    if nType and nType ~= Player.award_type_exp and nType ~= Player.award_type_basic_exp then
  ...
  local nPosX, nPosY = (FULL_WIDTH - nCurWidth) / 2, 45
  if nLine ~= 1 then
    nPosY = 70
  end
