...
local tbGoldCardShow = {
  [1] = true,
  [2] = true,
  [3] = true,
  [4] = true
}
local tbCoinCardShow = {
  [4] = true,
  [5] = true
}
...
function tbUi:InitCardInfo(pickerType)
  ...
  if pickerType == "Gold" then
    ...
    for nIdx, tbItem in ipairs(tbItems) do
      if tbItem.szItemType == CardPicker.Def.tbSpecialTenGoldSPartner.szItemType and tbItem.nItemId == CardPicker.Def.tbSpecialTenGoldSPartner.nItemId then
...
function tbUi:Update()
  ...
  if tbUi.bFirstOpenPreview then
    tbUi.bFirstOpenPreview = nil
  else
