...
tbGrid.tbOnClick = {}
function tbGrid.tbOnClick:BtnTake()
...
function tbUi:OnOpen(nGroupIndex)
  ...
  if nIdx and not tbSpecial[nIdx] then
    ...
    local nNormalGroupIndex = Recharge:GetAutoShowGrowInvest()
  ...
  local tbBtnTxt = {
    [4] = Recharge.tbGrowInvestActSetting.szNameInPanel,
    [7] = "回归专属"
  }
...
function tbUi:Update()
  ...
  self.pPanel:Label_SetText("Txt2", string.format("超值8倍返还%d元宝", nCanGet))
  local tbAllSetting = {}
  if not bBuyed then
  ...
  local function fnSetItem(tbItem, index)
