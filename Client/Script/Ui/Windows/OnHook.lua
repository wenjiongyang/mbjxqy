...
function OnHookPanel:Update()
  ...
  local nSpecialBJWExpTime = OnHook:CheckSpecialPayType(me) and (nExpTime < nSpecialBaiJuWanTime and nExpTime or nSpecialBaiJuWanTime)
  ...
  self.pPanel:SetActive("RemaindHostingTime", OnHook.OnHookTimePerDay < 86400)
  ...
  nOnHookToggle = nOnHookToggle or 2
  if nOnHookToggle == 3 and not OnHook:CheckSpecialBaiJuWanIsOpen(me) then
    nOnHookToggle = 2
  end
  self.pPanel:Toggle_SetChecked("Toggle2", nOnHookToggle == 2)
  self.pPanel:Toggle_SetChecked("Toggle1", nOnHookToggle == 1)
  self.pPanel:Toggle_SetChecked("Toggle3", nOnHookToggle == 3)
  ...
  local _, _, _, nQuality = KItem.GetItemShowInfo(OnHook.nBaiJuWanId, me.nFaction)
  local szIcon = Item.tbQualityColor[nQuality] or Item.DEFAULT_COLOR
  self.pPanel:Sprite_SetSprite("Color", szIcon)
  local _, _, _, nQuality1 = KItem.GetItemShowInfo(OnHook.nSpecialBaiJuWanId, me.nFaction)
...
function OnHookPanel:UpdateTip()
  ...
  local szTip = string.format("[FFFFFF]1、离线[-][FFFE0D]%s钟[-][FFFFFF]后自动进行离线托管，持续累计离线时间\n", Lib:TimeDesc8(OnHook.nDelayTime))
  szTip = szTip .. string.format("2、离线托管时间最多可累计至[-][FFFE0D]%d小时[-][FFFFFF]\n", OnHook.MaxExpTime / 3600)
  szTip = szTip .. string.format("[FFFFFF]3、达到[FFFE0D]等级上限且经验100%%[-][FFFFFF]后不可领取离线经验")
  self.pPanel:Label_SetText("LabelRules", szTip)
  local _, _, _, nQuality2 = KItem.GetItemShowInfo(OnHook.nBaiJuWanId, me.nFaction)
  local szIcon2 = Item.tbQualityColor[nQuality2] or Item.DEFAULT_COLOR
  self.pPanel:Sprite_SetSprite("Color2", szIcon2)
  local _, _, _, nQuality3 = KItem.GetItemShowInfo(OnHook.nSpecialBaiJuWanId, me.nFaction)
