...
function tbUi:RequesData()
  ...
  if tbFightTeam then
    ...
    if GetTime() - tbFightTeam.__RequesTime < 1800 then
      bReques = false
    end
  end
  if bReques then
    if self.nWLDHType then
      ...
      RemoteServer.DoRequesHSLJ("RequestFightTeamShow", self.nFightTeam)
    end
  end
end
...
function tbUi:SetPlayerInfo(nIndex, tbShowInfo)
  ...
  if tbShowInfo.nHonorLevel > 0 then
    ...
    self.pPanel:Sprite_Animation("PlayerTitle" .. nIndex, "Title" .. tbShowInfo.nHonorLevel .. "_")
  else
