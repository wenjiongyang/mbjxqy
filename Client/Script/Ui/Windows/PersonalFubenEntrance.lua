...
function tbUi:Update()
  ...
  self.pPanel:Sprite_SetFillPercent("BarRed", nRecommend <= nMyFightPower and 1 or nMyFightPower / nRecommend)
  self.pPanel:SetActive("BarGreen", nRecommend <= nMyFightPower)
...
function tbUi:Sweep(bConfirm)
  ...
  local tbParam = {
    nSectionIdx = self.nSectionIdx,
    nSubSectionIdx = self.nSubSectionIdx,
...
function tbUi:MultiSweep(bConfirm)
  ...
  local tbParam = {
    ...
    nSubSectionIdx = self.nSubSectionIdx,
    nFubenLevel = self.nFubenLevel,
    nSweepTimes = nAvailableTimesOrErrCode
  }
  if not bRet then
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_PERSONALFUBEN_TIMES_CHANGE,
      self.OnFubenTimesChange
    }
  }
  return tbRegEvent
end
...
function tbUi.tbOnClick:BtnJoinFuben()
...
function tbUi.tbOnClick:BtnSweep()
  self:Sweep()
end
function tbUi.tbOnClick:BtnSweep10()
  self:MultiSweep()
end
