...
function tbUi:UpdateNeedInfo()
  ...
  self.pPanel:Label_SetText("AwakenNeedTxt", string.format("下列[%s]（%s/5名）[-]%s级同伴%s", Lib:CountTB(self.tbSelectInfo) == 5 and "00FF00" or "FFFFFF", Lib:CountTB(self.tbSelectInfo), Partner.tbQualityLevelDes[self.tbPartner.nQualityLevel + 1], tbNeedInfo.bNeedWeapon and string.format("及其对应的[%s]（%s/5个）[-]本命武器", nWeaponCount == 5 and "00FF00" or "FFFFFF", nWeaponCount) or ""))
  ...
  local szItemName = KItem.GetItemShowInfo(Partner.nPartnerAwarenessCostItem)
...
function tbUi:GetAwarenessCanUsePartner(nPartnerTemplateId, bNeedWeapon)
  local tbAllPartner = me.GetAllPartner()
  local tbCanUsePartner, tbOtherPartner
  ...
  for nPId, tbInfo in pairs(tbAllPartner) do
    if not tbUsed[nPId] and tbInfo.nTemplateId == nPartnerTemplateId then
      if bNeedWeapon and not bHasWeapon and tbInfo.nWeaponState ~= 1 then
        ...
        table.insert(tbOtherPartner, {
          nPId,
          tbInfo.nFightPower
        })
      else
        tbCanUsePartner = tbCanUsePartner or {}
        table.insert(tbCanUsePartner, {
          nPId,
          tbInfo.nFightPower
        })
      end
...
function tbUi:OnClickPartner(nPos, nPartnerTemplateId, bNeedWeapon, bHasWeapon)
  if not bHasWeapon then
    local nWeaponItemId = Partner.tbPartner2WeaponItem[nPartnerTemplateId]
    local szName = KItem.GetItemShowInfo(nWeaponItemId or 0)
...
function tbUi.tbOnClick:BtnAwaken()
