...
function tbDamageCount:UpdateFlyChar()
  local szTime
  if 0 < (self.nDamageStartTime or 0) then
    local nTimeSpan = GetTime() - self.nDamageStartTime
    szTime = Lib:TimeDesc(nTimeSpan)
  else
    szTime = XT("计时:0分0秒")
  end
  local szMsg = string.format(XT("造成伤害:%d\n受到伤害:%d\n造成会心伤害:%d\n受到会心伤害:%d\n丢失:%d\n闪避:%d\n造成最大伤害:%d\n受到最大伤害:%d\n计时:%s\n"), self.nHitDamage, self.nHurtDamage, self.nHitDeadlyDamage, self.nHurtDeadlyDamage, self.nHitMiss, self.nHurtMiss, self.nHitMaxDamage, self.nHurtMaxDamage, szTime)
  self:ShowInfo(szMsg)
end
function tbDamageCount:OnUpdateFlyChar(nType, nValue)
  ...
  elseif (nType == emNPC_FLYCHAR_TYPE_HURT_NORMAL or nType == emNPC_FLYCHAR_TYPE_HURT_DEADLY) and nValue > self.nHurtMaxDamage then
...
function tbDamageCount:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYNC_FLY_CHAR,
      self.OnUpdateFlyChar
    }
  }
  return tbRegEvent
end
