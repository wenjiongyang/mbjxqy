...
function tbUi:UpdateFurniture(nFurnitureItemId)
  ...
  local tbMakeSetting = House.tbFurnitureMakeSetting[self.nFurnitureItemId] or {
    tbPosition = {
      0,
      0,
      0
    ...
    if itemObj then
      ...
      if tbCost[i] then
        ...
        if Player.AwardType[tbInfo[1]] == Player.award_type_item then
          ...
          local szColor = nNeedCount <= nCount and "[ffffff]" or "[ff0000]"
...
function tbUi:UpdateFurnitureTypeList()
  ...
  local function fnSetItem(itemObj, index)
    ...
    for i = 1, 2 do
      ...
      pObj.nType = nType
      function pObj.pPanel.OnTouchEvent(pObj)
...
function tbUi:OnSyncMakeFurniture(nFurnitureItemId, bResult)
  if not (bResult and nFurnitureItemId) or nFurnitureItemId ~= self.nFurnitureItemId then
  ...
  self.nFinishAniTimerId = Timer:Register(Env.GAME_FPS * tbUi.nAniTime, function()
    ...
    local szName = KItem.GetItemShowInfo(nFurnitureItemId, me.nFaction)
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYNC_MAKE_FURNITURE,
      self.OnSyncMakeFurniture
    },
    {
...
function tbUi.tbOnClick:BtnMake()
...
function tbUi.tbOnClick:BtnSelect()
...
function tbUi.tbOnClick:BtnOnlyShowCanMake()
...
function tbUi.tbOnClick:BtnTxt()
