...
function tbUi:UpdateItemType()
  ...
  for nIdx, pItem in ipairs(tbItem) do
    ...
    if tbClass.CheckUsable and tbClass:CheckUsable(pItem) == 0 then
      nSort = nSort + 500
    end
    local tbData = {
      nKey1 = nSort,
...
function tbUi:UpdateItemList()
  ...
  self.ScrollView:Update(math.max(#self.tbItemList, 6), fnSetItem)
end
function tbUi:OnSyncItem(nItemId, bUpdateAll)
  if bUpdateAll == 1 then
    self:UpdateItemType()
    self:UpdateItemList()
  elseif self.tbShowItem[nItemId] and self.tbShowItem[nItemId].nItemId == nItemId then
