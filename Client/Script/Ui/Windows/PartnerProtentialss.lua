...
function tbUi:Update(tbPartnerInfo, tbPAttribInfo)
  ...
  for i = 1, 4 do
    ...
    local nLimitProtential, bIsMaxGrade = Partner:GetLimitProtentialValue(tbPartnerInfo.nQualityLevel, tbPartnerInfo.nGrowthType, Partner.tbAllProtentialTypeStr2Id[szType], nLimitLevel, tbPartnerInfo.nGradeLevel + 1, Partner:GetPartnerAwareness(me, tbPartnerInfo.nTemplateId))
...
function tbUi:UpdateItemInfo()
  local nCount, tbItem = me.GetItemCountInAllPos(Partner.nPartnerProtentialItem)
...
function tbUi:OnSelectProtential(nIdx, nProtentialType)
  ...
  if not nCount or nCount <= 0 then
    MarketStall:TipBuyItemFromMarket(me, Partner.nPartnerProtentialItem)
