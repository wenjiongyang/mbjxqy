...
function tbUi:RegisterEvent()
  return {
    {
      UiNotify.emNOTIFY_LEVELUP_ASK4HELP_RSP,
      self.OnAsk4HelpRsp,
      self
    }
  }
...
local fnUpdateFriendItem = function(itemObj, tbPlayerInfo)
  ...
  local szHead, szAtlas = PlayerPortrait:GetSmallIcon(tbPlayerInfo.nPortrait or tbPlayerInfo.nFaction)
...
tbUi.tbUpdate = {
  Ask4Help = function(self)
    ...
    local function fnSetItem(itemObj, nIdx)
      ...
      function itemObj.BtnGive.pPanel.OnTouchEvent()
  ...
  HelpFriend = function(self)
    ...
    local nLastHelp = DirectLevelUp:GetLastHelpTimes(me)
    ...
    local function fnSetItem(itemObj, nIdx)
      ...
      function itemObj.BtnGive.pPanel.OnTouchEvent()
        ...
        me.MsgBox(string.format("只能赠送给一名好友，是否确定赠送给[FFFE0D]%s[-]？", tbPlayerInfo.szName), {
          {"确定", fnHelp},
          {"取消"}
        })
      end
