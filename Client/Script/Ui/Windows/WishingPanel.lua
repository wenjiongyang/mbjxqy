...
function tbUi:RegisterEvent()
  return {
    {
      UiNotify.emNOTIFY_WISHACT_DATA_CHANGED,
      self.OnDataUpadte
    }
  }
end
...
function tbUi:TryUpdateWishData()
  if not (self.dwKinId and self.dwKinId == me.dwKinId and self.nLastUpdateTime) or GetTime() - self.nLastUpdateTime >= 60 then
...
tbListUi.tbSetting = {
  WishList = {
    ...
    fnDone = function(self, szLabel)
      ...
      local function fnWish()
        if nWishType == Activity.WishAct.Wish_Type_Pay and me.GetMoney("Gold") < Activity.WishAct.nPayWishCost then
          me.CenterMsg("元宝不足")
          return
        end
  ...
  LabelList = {
    ...
    fnDone = function(self, szLabel)
      ...
      if nType == tbAct.PayLabel then
        me.MsgBox(string.format(" 是否花费%d元宝添加自定义祝福\n[FFFE0D]「" .. szLabel .. "」[-]", tbAct.nPayLabelCost), {
          {"确定", fnSend},
          {"取消"}
        })
      else
...
function tbListUi:OnOpenEnd(szType, ...)
  ...
  local function fnSetItem(itemObj, nIdx)
    ...
    function itemObj.pPanel.OnTouchEvent(btn)
