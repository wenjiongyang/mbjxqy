...
function tbUi:UpdateFightTeamInfo()
  ...
  if tbTeamInfo and tbTeamInfo.szName then
    ...
    self.pPanel:Label_SetText("TeamTime", string.format("%s/%s", tbTeamInfo.nJoinCount or 0, nMaxCount))
...
function HuaShanLunJian:RequestHSLJStateInfo(bForce)
  ...
  if not (not (nCurTime - self.nRequestTime > 180) or bRet) or bForce then
...
function HuaShanLunJian:IsOpenGameInTime(nTime)
  ...
  if nTime < nTFOpenTime or nMonthDay < tbDef.tbPrepareGame.nStartMonthDay or nMonthDay > tbDef.tbFinalsGame.nMonthDay then
    return false
  else
