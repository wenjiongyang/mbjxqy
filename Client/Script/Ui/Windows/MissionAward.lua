...
function tbUi:DoGetMoreAward(nGold)
  if nGold and nGold > me.GetMoney("Gold") then
    Timer:Register(1, function()
      me.MsgBox("元宝不足，是否前去充值？", {
        {
          "确认",
          Ui.OpenWindow,
          Ui,
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_MISSION_AWARD_ONRESULT,
      self.OnGetResult,
      self
    },
    {
      UiNotify.emNOTIFY_MISSION_AWARD_UPDATE,
      self.OnMissionUpdate,
      self
    }
  }
  return tbRegEvent
...
tbUi.tbOnClick = {}
function tbUi.tbOnClick:BtnGetAward()
  if not self.nType or not self.nRecordId then
    return
  end
  local nMaxAwardCount = MissionAward:GetMaxAwardTimes(me, self.nType)
  if nMaxAwardCount <= #self.tbAwardPos then
