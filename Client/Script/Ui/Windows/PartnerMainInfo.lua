...
function tbUi:OnSelectCoverSkill(nPartnerId, nItemId, nPos)
  ...
  if not (bRet and tbAllowInfo) or not tbAllowInfo[nPos] then
    return
  end
...
function tbUi.tbOnClick:BtnCompanionLevelUp()
  self:SetType(nil, tbPartnerUi.EXT_LEVELUP)
end
function tbUi.tbOnClick:BtnCompanionLearnSkill()
  self:SetType(nil, tbPartnerUi.EXT_LEARNSKILL)
end
function tbUi.tbOnClick:BtnCompanionSeverance()
...
function tbUi.tbOnClick:BtnLearn()
...
function tbUi.tbOnClick:BtnAddSkillBook()
  local tbItemList = {}
  local tbTmp = {}
