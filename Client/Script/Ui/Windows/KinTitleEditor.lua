...
function tbUi.tbOnClick:BtnSaveChange()
  ...
  for _, tbData in pairs(self.tbItemsData) do
    ...
    elseif tbData.szKinTitle ~= tbSpMap[tbData.nMemberId] then
...
tbEditorItem.tbOnSubmit = {}
function tbEditorItem.tbOnSubmit:TxtTitle()
  ...
  elseif szTitle == "" or string.len(szTitle) > Kin.Def.nMaxKinTitleLen or not bTitleAvailable then
    if not bTitleAvailable then
      me.CenterMsg(string.format("[%s]称谓不合法", szTitle))
    end
    local szCareerName = Kin.Def.Career_Name[nCareer]
