...
tbUi.tbActivityID = {Boss = 12, Leader = 13}
tbUi.tbTitleName = {
  Boss = "历代名将",
  Leader = "野外首领"
}
...
function tbUi:UpdateListItemMap(tbSubInfo)
  ...
  if tbSubInfo.nLinkMapID and 0 < tbSubInfo.nLinkMapID then
...
function tbUi:GetSyncState(tbTime)
  ...
  if nDaySec < tbTime[1] or nDaySec > tbTime[2] then
    return
  end
  return tbSyncData[3]
end
function tbUi:GetNpcDeathInfo(nMapTID, tbTime, tbAllNpcTID)
  ...
  for nTime, tbMapInfo in pairs(tbNpcDeath) do
    ...
    if nDaySec >= tbTime[1] and nDaySec <= tbTime[2] then
...
function tbUi:GetActivityState(tbTime)
  ...
  if nCurTime < tbTime[1] then
    return "NotStart"
  end
  if nCurTime > tbTime[2] then
    return "End"
  end
end
function tbUi:UpdateListLeft(tbListItem)
  ...
  local function fnSelLeftKey(tbItem)
...
function tbTimeSubItem:UpdateInfo(tbParentUi, tbSubInfo)
  ...
  local nTotalCount = #tbOpenTime
  if nTotalCount > 0 then
    local tbFindTimeInfo
    for nI, tbTimeInfo in ipairs(tbOpenTime) do
      tbFindTimeInfo = tbFindTimeInfo or tbTimeInfo
      local nCurTime = Lib:GetTodaySec()
      if nCurTime < tbTimeInfo[2] + 3600 then
