...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_ONCOMPOSE_CALLBACK,
...
function tbUi:Update()
  ...
  self.Details_Item:SetGenericItem({
    "Item",
    self.nTemplateID
  })
...
local GetTips = function(nTemplateID)
  local tbEquipInfo = KItem.GetPartnerItemInfo(nTemplateID)
  local szTips = "[ECE82B]"
  local fnCmp = function(tb1, tb2)
...
function tbUi:UpdatePartnerEquip()
  ...
  local bLevelEnough = nNeedLevel <= tbPartner.nLevel
  self.pPanel:SetActive("Tip", true)
  ...
  if nHaveNum > 0 then
    ...
    function self.tbOnClick.BtnEquip()
      self:Equip()
    end
...
function tbUi:UpdateContent()
  ...
  local szName = KItem.GetItemShowInfo(self.nCurItemID)
...
function tbUi:UpdateChildTree()
  ...
  for nIdx = 1, nChild do
    ...
    self["itemframe" .. nUiID]:SetGenericItem({
      "Item",
      tbCurChild.nChildTemplateID
    })
    ...
    local szColor = nNeedNum <= nHadNum and "[-]" or "[FF0000]"
...
tbUi.tbOnClick = {}
function tbUi.tbOnClick:BtnGoGain()
  self:OpenSubUi(GAIN)
end
function tbUi.tbOnClick:BtnGoCompose()
  self:OpenSubUi(COMPOSE)
end
function tbUi.tbOnClick:BtnCompose()
  RemoteServer.TryComposeItem(self.nCurItemID)
end
function tbUi.tbOnClick:BtnGainBack()
  local nTopNum = #self.tbTopItem
  if nTopNum > 1 then
