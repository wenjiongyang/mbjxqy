...
function tbMemberUi:Init(nIndex, tbMemberData, bCanHelp)
  ...
  local szHead, szAtlas = PlayerPortrait:GetPortraitBigIcon(tbMemberData.nPortrait)
...
function tbMemberUi.tbOnClick:TextureHead()
  ...
  Ui:OpenWindowAtPos("RightPopup", tbPos.x - 90, tbPos.y - 110, "Team", {
    dwRoleId = self.nPlayerId,
    dwKinId = self.nKinId
  })
end
function tbMemberUi.tbOnClick:BtnKickOut()
  ...
  me.MsgBox(string.format("是否将%s踢出队伍?", self.szName), {
    {"确定", fnConfirm},
    {"取消"}
  })
end
