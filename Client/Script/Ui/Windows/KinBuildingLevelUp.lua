...
function tbUi:InitSideBar()
  ...
  local function fnOnSelect(btnObj)
    Kin:UpdateBaseInfo()
  ...
  local function fnSetItem(itemObj, nIdx)
    ...
    function itemObj.pPanel.OnTouchEvent(btnObj)
      fnOnSelect(btnObj)
    ...
    if nMaxLevel > tbBuildingData.nLevel and bHasAuthority then
      local nUpgradeCost = Kin:GetBuildingUpgradeCost(nBuildingId, tbBuildingData.nLevel + 1)
      if nUpgradeCost <= Kin:GetFound() then
...
function tbUi:GetUpgradeDesc(nBuildingId, nNextLevel)
  ...
  tbBuildingInfo = tbBuildingInfo or Kin:GetBuildingInfo(nBuildingId, nNextLevel - 1) or {}
  ...
  if nBuildingId == Kin.Def.Building_FangJuHouse or nBuildingId == Kin.Def.Building_WeaponStore or nBuildingId == Kin.Def.Building_ShouShiHouse then
    ...
    if nNextOpenDay then
      szDesc = string.format("%s\n●  [FFFE0D]%d天后[-] 开放打造 [FFFE0D]%d阶装备[-]", szDesc, nNextOpenDay, nNextQuality)
    else
      local nCurQuality = Shop:EquipMakerGetCurMaxQuality()
      szDesc = string.format("%s\n●  当前打造已开放至 [FFFE0D]%d阶装备[-]", szDesc, nCurQuality)
    end
  end
  if 1 > self.nDiscountRate and self.nDiscountRate > 0 then
    local tbBuildingData = Kin:GetBuildingData(nBuildingId)
    local szAction = tbBuildingData.nLevel == 0 and "建造" or "升级"
    local szBuildingName = Kin:GetBuildingName(nBuildingId)
    szDesc = string.format("%s\n●  [FFFE0D]%s[-] 低于当前作坊等级上限，%s所需的建设资金为原来的 [FFFE0D]%d%%[-]", szDesc, szBuildingName, szAction, self.nDiscountRate * 100)
  end
...
function tbUi.tbOnClick:BtnLevelUp()
  ...
  if nMaxLevel < self.nNextLevel then
  ...
  Ui:OpenWindow("MessageBox", string.format(szInfo, Kin:GetBuildingName(self.nCurBuildingId)), {
    {fnAgree},
    {fnCancel}
  }, {"确定", "取消"})
end
