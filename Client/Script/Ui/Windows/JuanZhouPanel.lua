...
function tbUi:Update()
  ...
  local szName, nIcon, nView, nQuality = KItem.GetItemShowInfo(self.nTemplateId, me.nFaction)
  ...
  for idx, tbInfo in ipairs(self.tbSetting.tbNeedItem) do
    ...
    if nCount < tbInfo[2] then
      table.insert(tbUnFinishList, {
        "item",
        tbInfo[1],
        tbInfo[2]
      })
    else
      table.insert(tbFinishList, {
        "item",
        tbInfo[1],
        tbInfo[2]
      })
  ...
  local function fnSetItem(ItemObj, index)
    ...
    local szName, nIcon, nView, nQuality, szNameColor, szTip, nTotalNeedCount, nHaveCount, nNeedItemId
    if tbInfo[1] == "item" then
      szName, nIcon, nView, nQuality = KItem.GetItemShowInfo(tbInfo[2], me.nFaction)
    ...
    if nTotalNeedCount then
      local szNeedName = KItem.GetItemShowInfo(nNeedItemId)
      local szColor = nTotalNeedCount <= nHaveCount and "[00FF00]" or "[FFFFFF]"
      ItemObj.pPanel:Label_SetText("ComposeTxt", string.format("%s%s/%s个%s[-]", szColor, nHaveCount, nTotalNeedCount, szNeedName))
      function ItemObj.BtnCompose.pPanel.OnTouchEvent()
        local szDstName = KItem.GetItemShowInfo(tbInfo[2])
          ...
          me.MsgBox(string.format("确定合成[FFFE0D]%s[-]吗？花费[FFFE0D]%s银两[-]。", szDstName, nTotalCost), {
            {
              "确定",
              function()
                RemoteServer.DoQuickCombineStone(tbInfo[2])
              end
            },
            {"取消"}
          })
  ...
  self.ScrollView.pPanel:SetActive("Main", 0 >= self.tbSetting.nNeedCount)
  self.pPanel:SetActive("Tips", 0 < self.tbSetting.nNeedCount)
  if 0 < self.tbSetting.nNeedCount then
...
function tbUi.tbOnClick:BtnComplete()
  ...
  if self.tbSetting.nNeedCount <= 0 then
    ...
    local function fnCheckCanAdd(tbSelect, tbItem)
      ...
      if 0 < self.tbSetting.nNeedCount and nTotalCount >= self.tbSetting.nNeedCount then
        return false
      end
...
function tbUi.tbOnClick:BtnSell()
  Ui:CloseWindow(self.UI_NAME)
