...
tbRankPanel.tbOnClick = {
  ...
  BtnShop = function(self)
    Ui:OpenWindow("CommonShop", "Honor")
  end,
...
local StartFight = function(nFightType, nRankNo, nId)
...
function tbRankPanel:UpdateEnemy()
  ...
  local nSelfNo = RankBattle.DEF_NO
  ...
  local function fnSetItem(tbEnemy, index)
    local nRankNo = self.tbInfo[index].nRankNo
    if self.tbListId[nRankNo] == 1 or nSelfNo <= RankBattle.NO_LIMIT_RANK and nRankNo <= RankBattle.NO_LIMIT_RANK and nSelfNo ~= nRankNo then
