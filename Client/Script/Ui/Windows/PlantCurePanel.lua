...
function tbUi.tbOnClick:BtnAssist()
...
function tbUi:Refresh()
  ...
  local function fnSetRecord(tbRecord, nIndex)
    local tbInfo = tbRecordSet[nIndex]
    local nReduceTime = HousePlant.CURE_TIME_NORMAL
    local szTool = HousePlant.tbSickStateSetting[tbInfo.nState] and HousePlant.tbSickStateSetting[tbInfo.nState].szCureTool or "**"
    if tbInfo.bCost then
      nReduceTime = HousePlant.CURE_TIME_COST
      szTool = "高级" .. szTool
    end
...
function tbUi:RefreshTime(nLeftTime)
  ...
  if nLeftTime <= 0 then
    ...
    szTime = szTime .. Lib:TimeDesc2(nLeftTime)
  end
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_PLANT_CURE_FINISHED,
      function()
        self:Refresh()
      end
    },
