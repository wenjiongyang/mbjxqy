...
function tbUi:GetActTicketType(tbCurGuide)
  ...
  return Calendar:GetMarkTypeOfPlayer(szAct)
end
tbUi.tbMarkText = {
  "月度赛",
  "季度赛",
...
function tbUi:GetGuide()
  ...
  for _, nIdx in ipairs(self.tbTodayGuide or {}) do
    ...
    if me.nLevel >= tbInfo.nLevel then
      ...
      elseif szType == "OpenServer" or szType == "PreView" then
      ...
      elseif szType == "ActShow" and nCurTime >= tbParam[1] and nCurTime < tbParam[2] and Activity:__IsActInProcessByType(tbParam[3]) then
        return tbInfo
      end
    end
  end
...
function tbUi:AnalysicParam(tbGuide)
  ...
  elseif szType == "ActShow" then
    ...
    table.insert(tbAllParam, nBegin)
    table.insert(tbAllParam, nEnd)
    table.insert(tbAllParam, tbParam[1])
  end
  tbGuide.tbParam = tbAllParam
end
function tbUi:LoadFile()
  local tbFile = Lib:LoadTabFile("Setting/Guide/PlayerLevelUpGuide.tab", {
    nLevel = 1,
    nPreferential = 1,
    nCalendarId = 1,
    nLight = 1,
  ...
  table.sort(self.tbGuide, function(a1, a2)
    return a1.nPreferential > a2.nPreferential
  end)
end
tbUi:LoadFile()
