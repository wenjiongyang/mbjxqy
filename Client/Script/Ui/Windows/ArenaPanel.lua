...
function tbUi:RefreshUi()
  ...
  for nArenaId = 1, ArenaBattle.nArenaNum do
    ...
    if tbArenaInfo then
      ...
      self[szItem].pPanel:Sprite_Animation("PlayerTitle", "Title" .. nHonorLevel .. "_")
      local tbSetting = PlayerPortrait:GetPortraitSetting(nPortrait) or {}
      local szIcon = tbSetting.szIcon or ""
      local szAtlas = tbSetting.szIconAtlas or ""
...
function tbUi:RegisterEvent()
  local tbRegEvent = {
    {
      UiNotify.emNOTIFY_SYN_ARENA_DATA,
      self.RefreshUi,
      self
    }
  }
