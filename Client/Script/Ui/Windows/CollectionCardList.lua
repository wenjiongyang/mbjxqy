  ...
  local function fnSetItem(itemObj, nIdx)
    for i = 1, 5 do
      ...
      if nPos <= nShowLen then
        ...
        if bActivate then
          itemObj[szItem]:SetItemByTemplate(nCardTemplateId, 0, nil, {
            bShowCDLayer = not bActivate
          })
          itemObj[szItem].fnClick = itemObj[szItem].DefaultClick
        else
          local _, nIcon, _, nQuality = KItem.GetItemShowInfo(nCardTemplateId, me.nFaction)
