...
function CommerceTask:AutoPathToTaskNpc()
  local nPosX, nPosY = AutoPath:GetNpcPos(self.tbCommerceNpc.nNpcTemplateId, self.tbCommerceNpc.nMapTemplateId)
  local function fnCallback()
    local nNpcId = AutoAI.GetNpcIdByTemplateId(self.tbCommerceNpc.nNpcTemplateId)
    if nNpcId then
      Operation:SimpleTap(nNpcId)
    end
  end
  AutoPath:GotoAndCall(self.tbCommerceNpc.nMapTemplateId, nPosX, nPosY, fnCallback, Npc.DIALOG_DISTANCE)
end
function CommerceTask:LoadCommerceSetting()
  self.tbPosInfo = {}
  self.tbCommerceSetting = LoadTabFile("Setting/Npc/NpcCommerce.tab", "dss", "TemplateID", {
    "TemplateID",
    "Name",
    "Icon"
  })
...
function CommerceTask:LoadMapCommerceNpc(nMapTemplateId)
  ...
  local tbNpc = LoadTabFile(szPath, "ddd", nil, {
    "NpcTemplateId",
    "XPos",
    "YPos"
  })
  for _, tbNpcInfo in pairs(tbNpc) do
    ...
    if self.tbCommerceSetting[nTemplateId] then
      ...
      table.insert(self.tbPosInfo[nTemplateId], {
        nMapId = nMapTemplateId,
        nPosX = tbNpcInfo.XPos,
        nPosY = tbNpcInfo.YPos
      })
    end
  end
end
...
function CommerceTask:GetLastHelpTimes()
  ...
  if not (tbHelpOtherData and tbHelpOtherData.nHelpTimes and tbHelpOtherData.nLastHelpDay) or nCurDay > tbHelpOtherData.nLastHelpDay then
    return nHelpMax
  end
