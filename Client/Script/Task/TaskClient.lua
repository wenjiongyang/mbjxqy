...
function Task:InitMainTaskDes()
  local tbFile = LoadTabFile("Setting/Task/MainTaskInfo.tab", "dss", nil, {
    "nIndex",
    "szTitle",
    "szDesc"
  })
...
function Task:LoadClientNpc()
  local tbFile = LoadTabFile("Setting/Task/TaskClientNpc.tab", "ddddddsddss", nil, {
    "nShowId",
    "nNpcId",
    "nMapTemplateId",
    "nX",
...
function Task:CheckTaskNotSyncNpc(nMapTemplateId, nTaskId)
  ...
  for _, tbInfo in pairs(self.tbNpcNotSyncInfo[nMapTemplateId]) do
    if Task:GetTaskFlag(me, tbInfo.HideTaskId) ~= 1 then
      table.insert(tbNotSyncInfo, {
        nMapTemplateId,
        tbInfo.NpcTemplateId,
        tbInfo.XPos,
        tbInfo.YPos
      })
...
function Task:CheckClientNpc()
  ...
  if not (self.tbClientNpcInfo and self.tbClientNpcInfo.nMapTemplateId) or self.tbClientNpcInfo.nMapTemplateId ~= me.nMapTemplateId then
...
function Task:UpdateClientNpc(szTrap)
  ...
  for nShowId, nTaskId in pairs(tbShowIdx) do
    local tbShowNpcInfo = self.tbClientNpcInfo[nShowId]
    for _, tbNpcInfo in pairs(tbShowNpcInfo) do
      ...
      if pNpc then
        ...
        if fnStartAction and 10 < GetTime() - (pNpc.nActionTime or 0) then
...
function Task:SetNpcTaskState(pNpc, nState)
  ...
  local tbSkill = {
    [Task.STATE_ON_DING] = 1003,
    [Task.STATE_CAN_FINISH] = 1006,
    [Task.STATE_CAN_ACCEPT] = 1001
  }
...
function Task:OnAcceptTask(nTaskId)
  local tbPlayerTask = self:GetPlayerTaskInfo(me)
  table.insert(tbPlayerTask.tbCurTaskInfo, {
    nTaskId = nTaskId,
    tbState = {}
  })
  self:UpdateTaskInfo(nTaskId)
...
function Task:OnTrack(nTaskId)
  ...
  self:DoTrack(szTrackInfo, nTaskId)
end
...
function Task:UseTaskItem(nTaskId)
  ...
  local function fnOnGotoTaskItem()
    Timer:Register(10, function()
      RemoteServer.UseTaskItem(nTaskId)
    end)
  end
