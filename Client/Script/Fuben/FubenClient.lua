...
function Fuben:OnDialogCloseOpenUi()
  if self.szWaitOpenUi then
    Ui:OpenWindow(self.szWaitOpenUi)
...
function Fuben:OpenGuide(tbIns, szDescType, szDesc, szUiName, szWnd, tbPointer, bDisableClick, bBlackBg)
  if Ui:WindowVisible(szUiName) ~= 1 then
    Log("Error Guide:FubenOpenGuide", szUiName, szWnd)
    return
  end
  local tbWin = Ui(szUiName)
  Ui:OpenWindow("Guide", szDescType, szDesc, tbWin.pPanel, szWnd, tbPointer, bDisableClick, bBlackBg)
