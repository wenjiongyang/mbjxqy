...
function Kin:Ask4AutionData(szType)
  local tbAuction = tbClientAuctionData[szType] or {}
...
function Kin:PersonalAuctionAddedNotify(tbItems)
  ...
  for nIdx, tbItem in ipairs(tbItems) do
    local szItemName = KItem.GetItemShowInfo(tbItem.nItemId, me.nFaction)
  ...
  szInfo = string.format("恭喜获得%s，将以起拍价[FFFE0D]%d元宝[-]进行全服拍卖，成交后你将获得所有收益。\n[FFFE0D]（可在%d分钟之内前往拍卖取回物品）[-]", szInfo, nTotalPrice, Kin.AuctionDef.nPersonalAuctionWaitingTime / 60)
  local fnGo = function()
...
function Kin:AuctionPriceChanged(szType, nId, nPrice)
  ...
  Ui:OpenWindow("MessageBox", string.format("当前竞价已变动，你确定要花费 [FFFE0D]%d元宝[-] 参与竞拍[FFFE0D]【%s】[-]吗？", nPrice, szItemName), {
    {fnBid},
    {fnClose}
  }, {"确定", "取消"})
end
