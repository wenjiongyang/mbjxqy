...
function Kin:Create(szKinName, szAddDeclare, nCamp)
  ...
  RemoteServer.OnKinRequest("Create", szKinName, szAddDeclare, nCamp)
  return true
end
function Kin:CheckChangeName(szKinName)
  ...
  if nCost > me.GetMoney("Gold") then
    return false, "您的元宝不足"
  end
  local bValid, szErr = Kin:IsNameValid(szKinName)
  if not bValid then
...
function Kin:OnInvited(szInviter, szKinName, nKinId)
  Ui:OpenWindow("MessageBox", string.format("[FFFE0D]%s[-] 邀请你进入家族 [FFFE0D]%s[-]", szInviter, szKinName), {
    {
      function()
...
function Kin:ChangeCareer(nMemberId, nCareer, nReplaceMemberId, szName)
    ...
    if nCareer == Kin.Def.Career_Master then
      do
        ...
        if tbBaseInfo.szGroupOpenId then
          me.MsgBox("移交族长前必须解除家族群绑定, 是否确认解除?", {
            {
              "确定",
              function()
  ...
  if Kin:WillSendSalaryIn24Hours() and Kin:ShouldSalaryWarn({nReplaceMemberId}, {nMemberId}, nCareer) then
    if nReplaceMemberId and nReplaceMemberId > 0 then
      Ui:OpenWindow("MessageBox", "任职未满[FFFE0D]24小时[-]将无法获得工资，是否确定在此时替换职位成员？", {
        {fnConfirm},
        {}
      }, {"确定", "取消"})
    else
      Ui:OpenWindow("MessageBox", "小提示：任职未满[FFFE0D]24小时[-]将无法获得工资", {
        {fnConfirm},
        {}
      }, {
        "确定任命",
...
function Kin:AppointLeader(nCandidateId)
  ...
  if nDays >= 7 then
    me.MsgBox(string.format("「%s」离线超7天，确定继续任命吗？", tbData.szName), {
      {"确定", fnAppoint},
      {"取消"}
    })
  else
...
function Kin:ChangeAddDeclare(szAddDeclare)
  RemoteServer.OnKinRequest("ChangeAddDeclare", szAddDeclare)
end
...
function Kin:GetTitleSettingData()
  ...
  table.sort(tbTitleEditList, function(tbA, tbB)
    ...
    return tbA.nCareer < tbB.nCareer or tbA.nCareer == tbB.nCareer and tbA.nMemberId < tbB.nMemberId
  end)
  return tbTitleEditList
end
...
function Kin:IsNormalCareer(nCareer)
  return nCareer ~= Kin.Def.Career_New and nCareer ~= Kin.Def.Career_Retire
end
function Kin:GatherDrink()
  if me.nMapTemplateId ~= Kin.Def.nKinMapTemplateId then
    me.MsgBox("确定要回到家族参加活动吗？", {
      {
        "确定",
        Kin.GoKinMap
      },
  ...
  local szMsg = string.format("少侠确定要花费 [FFFE0D]60元宝[-] 请大伙喝酒吗？\n喝酒后可以获得[FFFE0D]%s[-]，篝火经验加成增加[FFFE0D]5%%%%[-]", tbAward[1])
...
function Kin:CanShowKinsStoreRed()
  ...
  return nOpenKinStoreDay ~= Lib:GetLocalDay(GetTime() - 14400)
end
...
function Kin:UpdateRedPoint()
  ...
  if tbApplyList and next(tbApplyList) and nApplyListSeenVersion ~= nVersion and Kin:CheckMyAuthority(Kin.Def.Authority_Recruit) then
...
local SortFunction = {
  ...
  SortByChuangGong = function(tbMemberList, bIsDown)
    ...
    table.sort(tbMemberList, function(a, b)
      ...
      local bAIsCan = bAIsOnline and ChuangGong:IsCanChuangGong(tbAParam)
      local bBIsCan = bBIsOnline and ChuangGong:IsCanChuangGong(tbBParam)
      local nAVip = a.nVipLevel or 0
      local nBVip = b.nVipLevel or 0
      if bIsDown then
        if bAIsCan and not bBIsCan then
          return true
        elseif not bAIsCan and bBIsCan then
          return false
        end
        if bAIsOnline and not bBIsOnline then
          return true
        elseif not bAIsOnline and bBIsOnline then
          return false
        end
        return nAVip > nBVip
      else
...
function Kin:UpdateGroupInfo(bForce)
  local nNow = GetTime()
  if self.nNextUpdateGroupInfoTime and nNow < self.nNextUpdateGroupInfoTime and not bForce then
    return
  end
  self.nNextUpdateGroupInfoTime = nNow + 30
