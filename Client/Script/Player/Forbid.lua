function Forbid:IsForbidAward()
  ...
  return nEndTime < 0 or nEndTime > 0 and nEndTime > GetTime()
end
...
function Forbid:GetDesc(bIsForbid)
  if bIsForbid then
    return string.format("由于[FFFE0D]%s[-]，你目前处于零收益状态。零收益状态下会被限制部分游戏功能，并且无法获得游戏奖励。距离状态解除还剩余时间 [FFFE0D]%s[-] ", self.szMsg, Lib:TimeDesc(self.nEndTime - GetTime()))
  else
    return "你已被解除零收益状态！"
  end
end
function Forbid:OnForbidAwardNotice()
  Ui:OpenWindow("MessageBoxBig", string.format("当前处于零收益状态，无法获得系统奖励。距离状态解除还剩余时间 [FFFE0D]%s[-] ", Lib:TimeDesc(self.nEndTime - GetTime())), {
    {}
  }, {"确定"}, 3)
end
