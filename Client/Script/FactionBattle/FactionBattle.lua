...
function FactionBattle:OnEnter()
  ...
  Ui:ChangeUiState(Ui.STATE_GAME_FIGHT, false)
end
...
function FactionBattle:OnEnterMap(nMapTemplateId)
  if nMapTemplateId ~= FactionBattle.PREPARE_MAP_TAMPLATE_ID and nMapTemplateId ~= FactionBattle.FREEPK_MAP_TAMPLATE_ID then
    self:OnLeave()
  end
  if nMapTemplateId == FactionBattle.PREPARE_MAP_TAMPLATE_ID or nMapTemplateId == FactionBattle.FREEPK_MAP_TAMPLATE_ID then
    Ui:ChangeUiState(Ui.STATE_GAME_FIGHT, false)
  end
end
...
function FactionBattle:OpenFactionBattleKingPanel()
  Ui:OpenWindow("NewInformationPanel", "FactionBattle")
end
function FactionBattle:OnWatchTrapOut()
  RepresentMgr.ClearShowRepNpc()
  UiNotify.OnNotify(UiNotify.emNOTIFY_REFRESH_QYH_BTN, "FactionBattle", {
    "BtnWitnessWar"
  })
end
...
function FactionBattle:GetWatchData()
  local tbWatch = {}
  tbWatch[1] = {
    FactionBattle.tbWatchInfo.players[1] or {}
  }
  tbWatch[2] = {
    FactionBattle.tbWatchInfo.players[2] or {}
  }
  local tbData = {
...
function FactionBattle:IsCanStart()
  return not HuaShanLunJian:IsOpenPreGameUi() and not HuaShanLunJian:IsOpenFinalsGameUi() and GetTimeFrameState("OpenLevel99") ~= 1 or Lib:GetLocalWeekDay() ~= 4
end
