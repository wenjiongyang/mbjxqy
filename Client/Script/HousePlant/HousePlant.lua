...
function HousePlant:OnSyncFriendPlant(tbData)
  ...
  for _, tbInfo in ipairs(tbData) do
    table.insert(tbSicks, {
      dwPlayerId = tbInfo.dwPlayerId,
      nState = tbInfo.nState,
      nImity = FriendShip:GetImity(me.dwID, tbInfo.dwPlayerId)
    })
  end
...
function HousePlant:OnClick(nRepId)
  ...
  if tbLand.nState == HousePlant.STATE_NULL then
    ...
    do
      ...
      local function fnPlant()
        me.MsgBox(string.format("确定要花费[FFFE0D]%d元宝[-]种下树丛吗？\n[FFFE0D]收成后可获得家具材料“木材”[-]", HousePlant.PLANT_COST), {
          {"确定", fnConfirm},
          {"取消", nil}
        })
      end
      ...
      Ui:CloseWindow("PlantStatePanel")
      Ui:OpenWindow("FurnitureOptUi", nRepId, {
        {"种植", fnPlant}
      }, fnClose, 80)
    end
  else
