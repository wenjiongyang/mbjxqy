...
function CardPicker:IsItemFlop(szItemType, nItemId, nShowLevel)
  if szItemType == "Partner" then
    ...
    if nQualityLevel and nShowLevel >= nQualityLevel then
      return true
    end
  end
  return false
...
function CardPicker:OnSyncPickHistory(tbHistory)
  ...
  for _, tbData in ipairs(tbHistory) do
    ...
    local szHistory = string.format("\n[FFFE0D]%s[-]在[FFFE0D]%s[-]中获得%s级同伴%s", szName, CardPicker.tbCardPickType2Name[szType] or "", Partner.tbQualityLevelDes[nQualityLevel], szPartnerName)
