...
function tbWebView:OpenUrlLunTan(szParam)
  ...
  local szUrl = ""
  if IOS then
    szUrl = string.format("%s?appid=%s&openid=%s&access_token=%s&acctype=%s&platid=%s&areaid=%s&roleid=%s&partition=%s", self.szLuanTanUrl, szAppid, szOpenID, szAccessToken, szAccountType, szPlatid, szAreaid, szRoleID, szPartition)
  else
    szUrl = string.format("%s?appid=%s&openid=%s&access_token=%s&acctype=%s&platid=%s&areaid=%s&roleid=%s&width=%s&height=%s&partition=%s", self.szLuanTanUrl, szAppid, szOpenID, szAccessToken, szAccountType, szPlatid, szAreaid, szRoleID, szWidth, szHeight, szPartition)
  end
...
function tbWebView:OnOrder_isMusicOn(szVarName)
  ...
  local nOpenSound = 0
  if tbUserSet.fSoundEffectVolume >= 0.1 or 0.1 <= tbUserSet.fMusicVolume then
    nOpenSound = 1
  end
