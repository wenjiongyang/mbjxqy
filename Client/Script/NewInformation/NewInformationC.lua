...
function NewInformation:Init()
  ...
  self.tbLocalSetting = {}
  local tbTabFile = Lib:LoadTabFile("Setting/NewInformation/TimingInformation.tab", {
    nReqLevel = 1,
    nShowPriority = 0,
    nBtnCloseWnd = 0
  })
...
function NewInformation:OnSettingChange(tbChangedSetting)
  if not next(tbChangedSetting) then
    return
  end
  local function fnReset(szKey, tbInfo)
...
function NewInformation:CheckRedPoint()
  ...
  for _, szKey in ipairs(tbRedPoint) do
    ...
    elseif tbActData and nClearSession < tbActData.nSession then
      bIsShowRedPoint = true
    end
    if nReqLevel <= me.nLevel and bIsShowRedPoint then
...
NewInformation.tbTimeJudge = {
  fnOpenServer = function(tbInfo)
    ...
    if string.find(szOpenTime, "|") then
      ...
      if bOpen and not bClose then
        ...
        return true, {
          nSession = Lib:GetLocalDay(nBeginTime),
          nValidTime = nOpenTime,
          tbData = {
            tbInfo.szContent
          }
        }
  ...
  fnSpecialTime = function(tbInfo)
    ...
    if nBegin <= nCurTime and nEnd >= nCurTime then
      return true, {
        nSession = Lib:GetLocalDay(nBegin),
        nValidTime = nEnd,
        tbData = {
          tbInfo.szContent
        }
  ...
  fnShowInWeek = function(tbInfo)
    ...
    if string.find(szShowWeek, nWeek) then
      ...
      local nValidTime = GetTime() - nTodaySec + 86400
      return true, {
        nSession = Lib:GetLocalWeek(),
        nValidTime = nValidTime,
        tbData = {
          tbInfo.szContent
        }
      }
...
function NewInformation:GetInfoDetail(szKey)
  ...
  if szActKeyName then
    ...
    return {
      szKey,
      tbActInfo.szTitle or "",
      tbActInfo.szUiName or "DragonBoatFestival"
    }
  end
end
