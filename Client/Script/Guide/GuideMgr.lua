local MakeFubenParam = function(nFubenLevel, nSectionId, nSubSectionId)
...
function Guide:Init()
  ...
  self.tbWaitingGuide = {}
  self.tbWaitingId = {}
  self.tbGuideSetting = LoadTabFile("Setting/Guide/GuideSetting.tab", "ddddss", "GuideId", {
    "GuideId",
    "SaveId",
    "CheckAchievement",
    "FightLimit",
  ...
  local tbSteps = LoadTabFile("Setting/Guide/GuideStep.tab", "ddsssssssd", nil, {
    "GuideId",
    "LoginStart",
    "ActionType",
    "Param",
...
function Guide:StartGuideById(nGuideId, bLoginStart, bForce)
  local tbSetting = self.tbGuideSetting[nGuideId]
  if not tbSetting then
    Log("Can not Start unexist Guide " .. nGuideId)
    return
  end
  self:StartGuide(tbSetting, bLoginStart, bForce)
end
function Guide:StartGuide(tbSetting, bLoginStart, bForce)
  if not bForce and self:IsFinishGuide(tbSetting.GuideId) ~= 0 then
    return
  end
  if 0 >= me.nMapTemplateId then
    return
  end
...
function Guide:OnCheckClickWndDestroy(nClickKey)
  if nClickKey == self.nCurClickKey then
    self.pCheckClickPanel = nil
...
function Guide:OnLogin()
  ...
  local tbCheckInLogin = {
    CheckLevel = function(nParam)
      return nParam <= nLevel
    end,
    CheckGuide = function(nParam)
      return Guide:IsFinishGuide(nParam) == 1
    end,
    CheckFuben = function(nParam)
      return nParam < (nParam % 2 == 0 and nEliteFubenId or nNormalFubenId)
    end,
    CheckTask = function(nParam)
      return false
    end
  }
