...
function tbNotifyGuide:LoadNotifyGuide()
  self.tbSetting = LoadTabFile("Setting/Guide/NotifyGuide.tab", "sddssds", "GuideName", {
    "GuideName",
    "SaveId",
    "StartLevel",
    "TimeFrame",
...
function tbNotifyGuide:LoginCheck()
  ...
  for szName, tbInfo in pairs(self.tbSetting) do
    if tbInfo.bLevelStart and nLevel >= tbInfo.StartLevel then
      self:StartNotifyGuide(szName)
    elseif tbInfo.nFubenStartKey and nCurFubenKey > tbInfo.nFubenStartKey then
      self:StartNotifyGuide(szName)
    end
  end
...
function tbNotifyGuide:ClearNotifyGuide(szName, bForce)
  ...
  elseif bForce and self:IsFinishGuide(szName) == 0 then
    RemoteServer.FinishNotifyGuide(szName)
  end
end
