...
function tbArborDayCure.FuncContent(tbData)
...
tbArborDayCure.tbSubInfo = {
  {
    szType = "Item2",
    szInfo = "活动内容：\n    活动期间，与一名[FFFE0D]亲密度5级以上的异性角色[-]组成[FFFE0D]二人队伍[-]去找[c8ff00][url=npc:真妹, 2204, 999][-]报名，[FFFE0D]成功报名[-]后通过邮件发放[FFFE0D]花草种子[-]以及[FFFE0D]养护道具[-]，男女拿到的道具不同，需注意[FFFE0D]一旦确定协作关系，活动期间不可更换与解除[-]！领取后需保持队伍前往双方的家园种下植物，每日固定时间植物会陷入虚弱状态，需两人组队各自进行治疗，[FFFE0D]点击植物可了解状态[-]，不同性别的角色需要治理的问题也不同。治疗时需注意花朵的对话内容，治疗错误将消耗次数且不计入养护成绩，养护成绩将决定活动结束时的最终奖励哦。",
    szBtnText = "前往",
    szBtnTrap = "[url=npc:text, 2204, 999]"
  }
}
...
function tbFathersDayAct.FuncContent(tbData)
  local tbTime1 = os.date("*t", tbData.nStartTime)
  local tbTime2 = os.date("*t", tbData.nEndTime + 1)
