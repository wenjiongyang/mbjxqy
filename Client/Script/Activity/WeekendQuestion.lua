...
tbActUi.szTitle = "开斋江湖动灵机"
tbActUi.szBtnText = "前往"
function tbActUi.fnCall()
  tbActUi:GoNpc()
end
...
function tbActUi:CheckTeamMember(pP)
  ...
  if not tbTeamMy then
    return false, "开斋江湖动灵机活动需要与[FFFE0D] 一名异性侠士 [-]组成队伍才能参加哦"
  end
  ...
  for nIdx, tbData in ipairs(tbTeamAll) do
    ...
    tbSecOK[nIdx] = Gift:CheckSex(nFaction)
  end
