...
function WeatherMgr:ChangeFog(nEnd, nFrame)
  ...
  Ui.ToolFunction.SetFogColor(0.011764705882352941, 0.03137254901960784, 0.13333333333333333)
...
function WeatherMgr:OnSyncWeatherInfo(nMapId, szWeatherType, nWeatherEndTime)
  ...
  if not (me.nMapId == nMapId and self.nMapId) or self.nMapId ~= nMapId then
    self.tbCacheWeather = {
      nMapId,
      szWeatherType,
      nWeatherEndTime
...
function WeatherMgr:ChangeWeather(nMapTemplateId, szWeatherType, nWeatherEndTime)
  if not (nMapTemplateId == self.nMapTemplateId and WeatherMgr.tbAllWeatherType[szWeatherType]) or nWeatherEndTime <= GetTime() then
    return
  end
  if self.bIsInSwitchDayNight then
...
WeatherMgr.Ui_ToolFunction_ChangeFog_Old = WeatherMgr.Ui_ToolFunction_ChangeFog_Old or Ui.ToolFunction.ChangeFog
function Ui.ToolFunction.ChangeFog(nDensity)
  WeatherMgr.RenderSettings = WeatherMgr.RenderSettings or luanet.import_type("UnityEngine.RenderSettings")
  WeatherMgr.FogMode = WeatherMgr.FogMode or luanet.import_type("UnityEngine.FogMode")
  if not WeatherMgr.RenderSettings.fog then
    WeatherMgr.RenderSettings.fogMode = WeatherMgr.FogMode.Exponential
    WeatherMgr.RenderSettings.fogDensity = 0
    WeatherMgr.RenderSettings.fog = true
  end
  WeatherMgr.RenderSettings.fogDensity = nDensity
  for nId, tbSetting in pairs(Decoration.tbClientDecoration) do
    local tbTemplate = Decoration.tbAllTemplate[tbSetting.nTemplateId]
    if tbTemplate and tbTemplate.szSubType == "Deng" then
      Decoration:OnSyncDecoration(tbSetting.nMapId, nId, tbSetting)
    end
  end
end
