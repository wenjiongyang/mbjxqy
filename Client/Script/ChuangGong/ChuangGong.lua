function ChuangGong:AcceptGetChuangGong(dwSendId)
  ...
  if not ChuangGong:IsWhiteMap(me.nMapTemplateId) and (Map:GetClassDesc(me.nMapTemplateId) ~= "fight" or me.nFightMode ~= 0) then
...
function ChuangGong:GoSafe(fnRequest, szMsg)
  ...
  me.MsgBox(szMsg or self.szRequestGoSafeTip, {
    {"确定", fnGoSafe},
    {"取消"}
  })
end
...
function ChuangGong:TryResetChuangGong()
  ...
  if nChuangGongTimes <= 0 and 0 < RegressionPrivilege:GetChuanGongTimes(me) then
